export const authApi = {
  // rdp
  // fetchAuthTreeDef: '/rdp/console/api/v1/auth/fetchAuthTreeDef',
  // listAuthDataByDataSource: '/rdp/console/api/v1/auth/listAuthDataByDataSource',
  // listAuthDataByDataTask: '/rdp/console/api/v1/auth/listAuthDataByDataTask',
  // authListLevels: '/rdp/console/api/v1/auth/listLevels',
  // modifyAuthData: '/rdp/console/api/v1/auth/modifyAuthData',
  // listCurrUserAuthResource: '/rdp/console/api/v1/auth/listCurrUserAuthResource',
  // listElementsOfLevel: '/rdp/console/api/v1/auth/listElementsOfLevel',
  // createAuthTicket: '/rdp/console/api/v1/ticket/createDataSourceAuthTicket',
  // listUserAuthOfRes: '/rdp/console/api/v1/auth/listUserAuthOfRes',
  // listUserAuthRes: '/rdp/console/api/v1/auth/listUserAuthRes',
  // listMyAuthOfRes: '/rdp/console/api/v1/auth/listMyAuthOfRes',
  // listMyAuthRes: '/rdp/console/api/v1/auth/listMyAuthRes',
  // modifyUserAuth: '/rdp/console/api/v1/auth/modifyUserAuth',
  // checkResourceManger: '/rdp/console/api/v1/auth/checkResourceManger',
  // listMyAuthResForCC: '/rdp/console/api/v1/auth/listMyAuthRes',

  //dm
  // listElementsOfLevelForDM: '/clouddm/console/api/v1/auth/listElementsOfLevel',
  // listLevelLeaf: '/clouddm/console/api/v1/auth/listElementOfLeaf',
  // listUserElementsOfLevelForDM: '/clouddm/console/api/v1/auth/listUserElementsOfLevel',
  // listUserLevelLeaf: '/clouddm/console/api/v1/auth/listUserElementOfLeaf',
  // listMyElementsOfLevelForDM: '/clouddm/console/api/v1/auth/listMyElementsOfLevel',
  // listMyLevelLeaf: '/clouddm/console/api/v1/auth/listMyElementOfLeaf',

  // cc
  // listElementsOfLevelForCC: '/cloudcanal/console/api/v1/inner/auth/listElementsOfLevel',
  // listUserAuthOfResForCC: '/cloudcanal/console/api/v1/inner/auth/listUserAuthOfRes',
  // listUserAuthResForCC: '/cloudcanal/console/api/v1/inner/auth/listUserAuthRes',
  // listMyAuthOfResForCC: '/cloudcanal/console/api/v1/inner/auth/listMyAuthOfRes',
  // modifyUserAuthForCC: '/cloudcanal/console/api/v1/inner/auth/modifyUserAuth',

  // rdp
  rdpAuthFetchAuthTreeDef: '/rdp/console/api/v1/auth/fetchAuthTreeDef',
  rdpAuthListElementsOfLevel: '/rdp/console/api/v1/auth/listElementsOfLevel',
  rdpAuthListUserAuthRes: '/rdp/console/api/v1/auth/listUserAuthRes',
  rdpAuthListMyAuthOfRes: '/rdp/console/api/v1/auth/listMyAuthOfRes',
  rdpAuthListMyAuthRes: '/rdp/console/api/v1/auth/listMyAuthRes',
  rdpAuthModifyUserAuth: '/rdp/console/api/v1/auth/modifyUserAuth',
  rdpAuthCheckResourceManger: '/rdp/console/api/v1/auth/checkResourceManger',
  rdpAuthListUserAuthOfRes: '/rdp/console/api/v1/auth/listUserAuthOfRes',

  //dm
  dmAuthListElementsOfLevel: '/clouddm/console/api/v1/auth/listElementsOfLevel',
  dmAuthListElementOfLeaf: '/clouddm/console/api/v1/auth/listElementOfLeaf',
  dmAuthListUserElementsOfLevel: '/clouddm/console/api/v1/auth/listUserElementsOfLevel',
  dmAuthListUserElementOfLeaf: '/clouddm/console/api/v1/auth/listUserElementOfLeaf',
  dmAuthListMyElementsOfLevel: '/clouddm/console/api/v1/auth/listMyElementsOfLevel',
  dmAuthListMyElementOfLeaf: '/clouddm/console/api/v1/auth/listMyElementOfLeaf',

  // cc
  ccAuthListElementsOfLevel: '/cloudcanal/console/api/v1/inner/auth/listElementsOfLevel',
  ccAuthListUserAuthOfRes: '/cloudcanal/console/api/v1/inner/auth/listUserAuthOfRes',
  ccAuthListUserAuthRes: '/cloudcanal/console/api/v1/inner/auth/listUserAuthRes',
  ccAuthListMyAuthOfRes: '/cloudcanal/console/api/v1/inner/auth/listMyAuthOfRes',
  ccAuthModifyUserAuth: '/cloudcanal/console/api/v1/inner/auth/modifyUserAuth',
  ccAuthListCatalog: '/cloudcanal/console/api/v1/auth/listcatalog'
};
