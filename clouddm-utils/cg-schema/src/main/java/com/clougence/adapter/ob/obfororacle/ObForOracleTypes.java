...
import com.clougence.adapter.ob.obfororacle.driver.OracleType;
 ...
 */
public enum ObForOracleTypes implements FieldType {
    ...
    ObForOracleTypes(String codeKey, OracleType oracleType){
        this.codeKey = codeKey;
        this.jdbcType = oracleType.toJdbcType();
        this.jdbcTypeNum = oracleType.getVendorTypeNumber();
    }

    public static ObForOracleTypes valueOfCode(String code) {
        code = code.toUpperCase();
        if (AliasNames.containsKey(code)) {
            code = AliasNames.get(code);
        }

        for (ObForOracleTypes tableType : ObForOracleTypes.values()) {
    ...
    @Override
    public DsType getDsType() { return DsType.ObForOracle; }
    ...
    public static ObForOracleTypes toOracleType(String dataType, Integer numPrecision, Integer numScale) {
        ...
        if (dataType.startsWith("NUMBER")) {
            if (numPrecision != null && numPrecision > 0 && (numScale == null || numScale == 0)) {
                return ObForOracleTypes.NUMBER_BIGINT;
            } else {
                return ObForOracleTypes.NUMBER_DECIMAL;
            }
        ...
        //
        for (ObForOracleTypes type : ObForOracleTypes.values()) {
