...
import com.clougence.adapter.polar.pormy.PolarDBMyOnCurrentUpdateType;
 ...
 */
public class PolarDBMyTableImporter extends AbstractRdbTableImporter {
    ...
    private void processIdxWithText(TableEditor tableEditor) {
        ...
        for (String indexName : indexNames) {
            ...
            for (String column : columnList) {
                ...
                PolarDBMyTypes type = PolarDBMyTypes.valueOfCode(editorColumn.getSource().getDbType());
            ...
            if (hasLob) {
                ...
                INDEX_TYPE.setValue(indexEditor.getSource().getAttribute(), PolarDBMyIndexType.FullText.name());
            }
        }
    }
    ...
    public void afterMapping(TableEditor tableEditor, ColumnEditor columnEditor, TriggerContext triggerContext, //
        ...
        PolarDBMyTypes sqlTypes = PolarDBMyTypes.valueOfCode(domain.getDbType());

        // CHAR / VARCHAR default has value but length out of size
        processDefaultValueLengthOutOfColumnLength(sqlTypes, columnEditor);

        if (PolarDBMyMainVersion.PolarDBMy_5_6.isEq(dstMainVersion)) {
        ...
        } else if (PolarDBMyMainVersion.PolarDBMy_5_7.isEq(dstMainVersion)) {
        ...
        } else if (PolarDBMyMainVersion.PolarDBMy_8_0.isEq(dstMainVersion)) {
    ...
    private void processDefaultValueLengthOutOfColumnLength(PolarDBMyTypes sqlTypes, ColumnEditor columnEditor) {
        EColumn domain = columnEditor.getSource();
    ...
    private void processTimesFuncHaveDefaultOrUpdate(PolarDBMyTypes sqlTypes, ColumnEditor columnEditor) {
        EColumn domain = columnEditor.getSource();
        String defaultValue = domain.getDefaultValue();
        ...
        boolean isDate = sqlTypes == PolarDBMyTypes.TIMESTAMP || sqlTypes == PolarDBMyTypes.DATE || sqlTypes == PolarDBMyTypes.TIME || sqlTypes == PolarDBMyTypes.DATETIME;
        ...
        switch (sqlTypes) {
            case TIME:
            case DATE: {
                columnEditor.changeType(PolarDBMyTypes.TIMESTAMP.getCodeKey());
                ...
                if (StringUtils.isNotBlank(updateTypeValue)) {
                    CURRENT_UPDATE_TYPE.setValue(domain.getAttribute(), PolarDBMyOnCurrentUpdateType.CurrentTimestamp.getTypeName());
                }
            }
            default:
                break;
    ...
    private void processNumberDefaultHasFuncIgnore(PolarDBMyTypes sqlTypes, ColumnEditor columnEditor) {
        EColumn domain = columnEditor.getSource();
        String defaultValue = domain.getDefaultValue();
        boolean defaultValueIsFunc = domain.isDefaultValueIsFunc();
    ...
    private void processTimestampStrategy(TableEditor tableEditor, TriggerContext triggerContext) {
        ...
        List<String> timestampColumns = tableEditor.getSource().getColumnList().stream().filter(eColumn -> {
            PolarDBMyTypes sqlTypes = PolarDBMyTypes.valueOfCode(eColumn.getDbType());
            return sqlTypes == PolarDBMyTypes.TIMESTAMP;
    ...
    private void processMaxKeySize(TableEditor tableEditor, TriggerContext triggerContext, MainVersion dstMainVersion) {
        ...
        if (aliSql) {
            if (dstMainVersion.isEq(MySQLMainVersion.MySQL_5_6) || dstMainVersion.isEq(MySQLMainVersion.MySQL_5_7)) {
                processMaxKeySize(tableEditor, 767, 191);
            } else if (dstMainVersion.isEq(MySQLMainVersion.MySQL_8_0)) {
                processMaxKeySize(tableEditor, 3072, 768);
            }
        } else {
            if (dstMainVersion.isEq(MySQLMainVersion.MySQL_5_6)) {
                processMaxKeySize(tableEditor, 767, 191);
            } else if (dstMainVersion.isEq(MySQLMainVersion.MySQL_5_7) || dstMainVersion.isEq(MySQLMainVersion.MySQL_8_0)) {
                processMaxKeySize(tableEditor, 3072, 768);
            }
        }
    ...
    private void processMaxKeySize(TableEditor tableEditor, int defaultMaxKeySize, int partKeySize) {
        ...
        for (String index : tableEditor.getIndexes()) {
            ...
            for (String column : columnList) {
                ...
                PolarDBMyTypes types = PolarDBMyTypes.valueOfCode(columnEditor.getSource().getDbType());
    ...
    private void processColumns(TableEditor tableEditor, long rowOverFlowSize) {
        ...
        Map<String, PolarDBMyTypes> columnTypes = new HashMap<>();
        ...
        for (String columnName : tableEditor.getColumnNames()) {
            ...
            PolarDBMyTypes types = PolarDBMyTypes.valueOfCode(eColumn.getDbType());
        ...
        needProcess.sort((o1, o2) -> {
            PolarDBMyTypes t1 = columnTypes.get(o1.getSource().getName());
            PolarDBMyTypes t2 = columnTypes.get(o2.getSource().getName());
        ...
        for (ColumnEditor column : needProcess) {
            ...
            PolarDBMyTypes type = columnTypes.get(eColumn.getName());
            ...
            if (isChars(type)) {
                column.changeType(PolarDBMyTypes.TEXT.getCodeKey());
                rowOverFlowSize = rowOverFlowSize - typeSize + 20;
            }
            if (isBinary(type)) {
                column.changeType(PolarDBMyTypes.BLOB.getCodeKey());
                rowOverFlowSize = rowOverFlowSize - typeSize + 20;
            }
    ...
    private boolean isLob(PolarDBMyTypes type) {
        switch (type) {
            case BLOB:
            case TINYBLOB:
            case MEDIUMBLOB:
    ...
    private boolean isChars(PolarDBMyTypes type) {
        switch (type) {
            case CHAR:
            case VARCHAR:
                return true;
    ...
    private boolean isBinary(PolarDBMyTypes type) {
        switch (type) {
            case BIT:
            case BINARY:
            case VARBINARY:
    ...
    private long typeOfSize(PolarDBMyTypes type, EColumn eColumn) {
    ...
    protected FieldType parseFieldTypeByColumnType(String columnType) {
        return PolarDBMyTypes.valueOfCode(columnType);
    }
}
