 ...
 */
public class PolarDbMyTableExporter implements TableBeforeMappingHandler, ColumnBeforeMappingHandler {
    ...
    private void bitUpgrade(ColumnEditor columnEditor, DsType dstType) {
        ...
        PolarDBMyTypes polarDbMyTypes = PolarDBMyTypes.valueOfCode(eColumn.getDbType());
        if (polarDbMyTypes != PolarDBMyTypes.BIT) {
            return;
        }
        ...
        if (numericPrecision != null && numericPrecision == 1) {
            columnEditor.changeType(PolarDBMyTypes.TINYINT.getCodeKey());
            columnEditor.setDefault(defaultValue, false);
        } else {
            columnEditor.changeType(PolarDBMyTypes.TEXT.getCodeKey());
            columnEditor.setDefault(defaultValue, false);
        }
    }
}
