...
public class WrapArrayBufferTest {
    ...
    public void basicTest01() {
        BytesIO io = BytesIO.wrap(new byte[111]);
        assert io.capacity() == 111;
        assert io.toString().startsWith("WrapArrayBytesIO[rMark=");
        io.free();
    }

    @Test
    public void basicTest02() {
        BytesIO io = BytesIO.wrap(new byte[] { 1, 2, 3, 4 });
    ...
    public void basicTest03() {
        ...
        BytesIO io = BytesIO.wrap(new byte[1024], true);
        assert io.writeBytes(cacheData) == 1024;
    }

    @Test
    public void basicTest04() {
        BytesIO io = BytesIO.wrap(new byte[4], true);
    ...
    public void basicTest05() {
        BytesIO wrap = BytesIO.wrap(new byte[] { 1, 2 });
    ...
    public void freeTest01() {
        BytesIO io = BytesIO.wrap(new byte[111], true);
        io.free();
    ...
    public void copyTest01() throws NoSuchAlgorithmException {
        WrapArrayBytesIO io1 = (WrapArrayBytesIO) BytesIO.wrap(new byte[] { 1, 2, 3, 4 });
        WrapArrayBytesIO io2 = io1.copy();
    ...
    public void copyTest02() {
        BytesIO io1 = BytesIO.wrap(new byte[] { 1, 2, 3, 4 });
    ...
    public void errorTest01() {
        try {
            BytesIO.wrap((byte[]) null);
            assert false;
        } catch (NullPointerException e) {
            assert e.getMessage().equals("bytes is null.");
        }
    }
