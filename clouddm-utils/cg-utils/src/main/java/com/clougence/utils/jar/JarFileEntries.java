...
class JarFileEntries implements CentralDirectoryVisitor, Iterable<JarEntry> {
    ...
    RandomFile getEntryData(String name) throws IOException {
        FileHeader entry = getEntry(name, FileHeader.class, false);
        if (entry == null) {
            return null;
        }
    ...
    private RandomFile getEntryData(FileHeader entry) throws IOException {
        // aspectjrt-1.7.4.jar has a different ext bytes length in the
        // local directory to the central directory. We need to re-read
        // here to skip them
        RandomFile data = this.jarFile.getData();
    ...
    private <T extends FileHeader> T getEntry(int index, Class<T> type, boolean cacheEntry, AsciiBytes nameAlias) {
        try {
            ...
            FileHeader entry = (cached != null) ? cached : CentralDirectoryFileHeader.fromRandomFile(this.centralDirectoryData, this.centralDirectoryOffsets[index], this.filter);
