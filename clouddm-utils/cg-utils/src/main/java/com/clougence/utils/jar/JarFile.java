 ...
 */
package com.clougence.utils.jar;
...
public class JarFile extends AbstractJarFile implements Iterable<java.util.jar.JarEntry> {
    ...
    private static final String                  HANDLERS_PACKAGE         = "com.clougence.utils.loader";
     ...
     * @throws IOException if the file cannot be read
     */
    JarFile(RandomFile file) throws IOException {
     ...
     * @throws IOException if the file cannot be read
     */
    private JarFile(RandomFile rootFile, String pathFromRoot, RandomFile data, JarFileType type) throws IOException {
        this(rootFile, pathFromRoot, data, null, type, null);
    }

    private JarFile(RandomFile rootFile, String pathFromRoot, RandomFile data, JarEntryFilter filter, JarFileType type, Supplier<Manifest> manifestSupplier) throws IOException {
    ...
    private CentralDirectoryVisitor centralDirectoryVisitor() {
        return new CentralDirectoryVisitor() {
            @Override
            public void visitStart(CentralDirectoryEndRecord endRecord, RandomFile centralDirectoryData) {
    ...
    protected final RandomFile getRootJarFile() {
    ...
    public Stream<java.util.jar.JarEntry> stream() {
        Spliterator<java.util.jar.JarEntry> spliterator = Spliterators.spliterator(iterator(), size(), Spliterator.ORDERED | Spliterator.DISTINCT | Spliterator.IMMUTABLE | Spliterator.NONNULL);
    ...
    private JarFile createJarFileFromFileEntry(JarEntry entry) throws IOException {
        ...
        RandomFile entryData = this.entries.getEntryData(entry.getName());
