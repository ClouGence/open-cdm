...
import com.clougence.utils.io.output.StringBuilderWriter;
...
public class IOUtils {
    ...
    public static void closeQuietly(final Closeable closeable, EConsumer<IOException, IOException> consumer) throws IOException {
        try {
            if (closeable != null) {
                closeable.close();
            }
    ...
    public static String readToString(Reader input) throws IOException {
        return readToString(input, (long) -1);
    }

    public static String readToString(Reader input, int len) throws IOException {
        return readToString(input, (long) len);
    }

    public static String readToString(Reader input, long len) throws IOException {
        if (input == null) {
            return null;
        } else if (len == 0) {
            return "";
        ...
        if (len < 0) {
            ...
            long readLen = len;
            while ((charsRead = input.read(buffer)) != -1) {
                if (readLen >= charsRead) {
                    builder.append(buffer, 0, charsRead);
                    readLen -= charsRead;
                } else {
                    builder.append(buffer, 0, (int) readLen);
                    break;
                }
            }
        }
