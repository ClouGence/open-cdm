...
/**
 * {@link RandomFile} implementation backed by a {@link RandomAccessFile}.
 *
 * @author Phillip Webb
...
public class RandomFile {
    ...
    /**
     * Create a new {@link RandomFile} backed by the specified file.
     ...
     */
    private RandomFile(FileAccess fileAccess, long offset, long length) {
    ...
    public RandomFile getSubsection(long offset, long length) {
        if (offset < 0 || length < 0 || offset + length > this.length) {
            throw new IndexOutOfBoundsException();
        }
        return new RandomFile(this.fileAccess, this.offset + offset, length);
    }
    ...
    /** {@link InputStream} implementation for the {@link RandomFile}. */
    private class DataInputStream extends InputStream {
        private int position;

        public int read() throws IOException {
            int read = RandomFile.this.readByte(this.position);
            if (read > -1) {
                moveOn(1);
            }
            return read;
