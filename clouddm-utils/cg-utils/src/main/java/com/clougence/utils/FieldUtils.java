 ...
 *
 * 2021-11-01 reduction based on Apache Commons lang3, org.apache.commons.lang3.reflect.FieldUtils
 *
...
public class FieldUtils {
            ...
            try {
                ...
                if (match != null) {
                    throw new IllegalArgumentException(
                        "Reference to field " + fieldName + " is ambiguous relative to " + cls + "; a matching field exists on two or more implemented interfaces.");
                }
                match = test;
            } catch (NoSuchFieldException ex) {
                // ignore
    ...
    /**
     * append [] at the ends of field string such that:
     * <blockquote><pre>
     *     // result is varchar(255)[][][]
     *     FieldUtils.transformToArrayFieldString("varchar(255)", 3)
     *     // result is int[][]
     *     FieldUtils.transformToArrayFieldString("int", 2)
     * </pre></blockquote>
     *
     * @param fieldType field type which need transform to fieldType
     * @param dimensions array dimensions
     * @throws IllegalArgumentException if <code>dimensions <= 0</code> or fieldType is null or blank
     * @return result after append
     */
    public static String transformToArrayFieldString(final String fieldType, final Integer dimensions) {
        if (dimensions == null || dimensions < 0) {
            throw new IllegalArgumentException("dimensions must not be < 0 or null");
        }
        if (fieldType == null || fieldType.isEmpty()) {
            throw new IllegalArgumentException("fieldType must not be null");
        }
        StringBuilder tgb = new StringBuilder(fieldType);
