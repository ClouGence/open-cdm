...
class SaxXmlParser extends DefaultHandler {
    ...
    public void startElement(final String uri, final String localName, final String qName, final Attributes attributes) {
        String elementName = StringUtils.defaultIfBlank(localName, qName);
        TreeNode treeNode = this.newNode(uri, elementName);
        for (int i = 0; i < attributes.getLength(); i++) {
            String attName = StringUtils.defaultIfBlank(attributes.getLocalName(i), attributes.getQName(i));
