...
public abstract class ResultSetInputStream extends InputStream {
    ...
    private DataHeader getDataHeader(long offset) throws IOException {
        ...
        if (lenFieldBytes == 0) {
            return new DataHeader(typeCode, offset, 2, 0, offset + 2, true);
        }

        // dataLen header
        long dataLen = this.getLengthData(offset + 2, lenFieldBytes);
        long dataPos = offset + 2 + lenFieldBytes;
        ...
        return new DataHeader(typeCode, offset, dataHeaderLen, dataLen, dataPos, false, charsetName);
    }
    ...
    public static class DataHeader {
        ...
        DataHeader(byte typeCode, long position, long headerLength, long dataBodyLength, long dataBodyPos, boolean dataBodyIsNull){
            this(typeCode, position, headerLength, dataBodyLength, dataBodyPos, dataBodyIsNull, null);
        }

        DataHeader(byte typeCode, long position, long headerLength, long dataBodyLength, long dataBodyPos, boolean dataBodyIsNull, String charsetName){
