...
```bash
# One-click startup
docker run -d --name cgdm-alone -p 8222:8222 bladepipe/cgdm-alone:4.2.0
...
services:
  dm_alone:
    image: clougence/cgdm-alone:4.2.0
...
docker run -d --name dm_sidecar \
  ...
  -e APP_SERVE_NAME=dm_console \
  -e APP_SERVE_PORT=8008 \
  bladepipe/cgdm-sidecar:4.2.0
```

For China deployment, simply replace the images with:

- `cloudcanal-registry.cn-shanghai.cr.aliyuncs.com/clougence/cgdm-console:4.2.0`
- `cloudcanal-registry.cn-shanghai.cr.aliyuncs.com/clougence/cgdm-sidecar:4.2.0`
...
services:
  ...
  dm_console:
    image: clougence/cgdm-console:x86_64-4.2.0
  ...
  dm_sidecar:
    image: clougence/cgdm-sidecar:x86_64-4.2.0
