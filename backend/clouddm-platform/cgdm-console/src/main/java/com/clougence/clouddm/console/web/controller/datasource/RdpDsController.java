...
public class RdpDsController {
    ...
    public ResWebData<?> listByCondition(@RequestBody @Valid ListDsFO listDsFO, HttpServletRequest request) {
        ...
        List<DmAuthResDO> authList = this.authServiceForBiz.listAuthByUser(uid, AuthKind.DataSource);
    ...
    private List<RdpSimpleDsVO> genAndFilterToSimpleVO(List<DmDsDO> dsDOs, ListDsFO listDsFO) {
        ...
        vos = dsDOs.stream().map(this::genRdpSimpleDsVO).collect(Collectors.toList());
        return vos;
    }
    ...
    public ResWebData<?> queryDs(@RequestBody @Valid QueryDsFO queryDsFO, HttpServletRequest request) {
        ...
        this.authServiceForBiz.checkResAuth(puid, uid, queryDsFO.getDataSourceId(), RdpAuthUtils.genEmptyResPath(), RDP_DAUTH_DS_READ, AuthKind.DataSource);

        DmDsDO result = this.dsService.queryDsByIdWithoutPasswd(queryDsFO.getDataSourceId());
        auditService.logAndAddOperationAudit(puid, uid, request.getRequestURI(), request.getRemoteAddr(), queryDsFO
    ...
    public ResWebData<DriverVersionStatusVO> checkDriverStatus(@RequestBody @Valid CheckDriverVersionFO fo) {
        DriverVersionStatusVO statusVO = this.driverService.checkDriverStatus(fo.getClusterId(),//
    ...
    public ResWebData<?> downloadDriver(@RequestBody @Valid CheckDriverVersionFO fo, HttpServletRequest request) {
        String uid = (String) request.getAttribute(RdpUserService.UID);
        this.driverService.downloadDriver(uid, fo.getClusterId(),//
                fo.getDriverFamily(), fo.getDriverVersion());
    ...
    public ResWebData<Long> addDs(@RequestParam("DataSourceAddData") String data, //
        ...
        ResWebData<Long> longResWebData = this.dsService.addDataSource(puid, uid, addDsFO);
        this.auditService.logAndAddOperationAudit(puid, uid, request.getRequestURI(), request.getRemoteAddr(), longResWebData
    ...
    public ResWebData<?> delDs(@RequestBody @Valid DeleteDsFO deleteDsFO, HttpServletRequest request) {
        ...
        this.authServiceForBiz.checkResAuth(puid, uid, resId, RdpAuthUtils.genEmptyResPath(), RDP_DAUTH_DS_MANAGER, AuthKind.DataSource);

        DmDsDO rdpDataSourceDO = dmDsService.fetchAndCheckById(resId);
        ResWebData<Long> longResWebData = this.dsService.delDataSource(puid, resId);
    ...
    public ResWebData<?> queryDsConfig(@RequestBody QueryDsConfigFO fo, HttpServletRequest request) {
        ...
        this.authServiceForBiz.checkResAuth(puid, uid, resId, RdpAuthUtils.genEmptyResPath(), RDP_DAUTH_DS_READ, AuthKind.DataSource);

        List<RdpDsKvConfigVO> vos = this.dsService.queryDsConfigs(fo.getDataSourceId());
        auditService.logAndAddOperationAudit(puid, uid, request.getRequestURI(), request.getRemoteAddr(), fo
    ...
    public ResWebData<?> upsertDsConfig(@RequestBody UpsertDsKvConfigFO fo, HttpServletRequest request) {
        ...
        List<UpdateDsConfigLO> configLOS = this.dsService.upsertDsConfigs(puid, fo);
        this.auditService.logAndAddOperationAudit(puid, uid, request.getRequestURI(), request.getRemoteAddr(), fo
    ...
    public ResWebData<?> updateDataSourceDesc(@RequestBody @Valid UpdateDsDescFO fo, HttpServletRequest request) {
        ...
        UpdateDsDescLO dsDescLO = this.dsService.updateDataSourceDesc(puid, fo.getDataSourceId(), fo.getInstanceDesc());
        auditService.logAndAddOperationAudit(puid, uid, request.getRequestURI(), request.getRemoteAddr(), fo
    ...
    public ResWebData<?> updateAccountAndPassword(@RequestParam("DataSourceUpdateData") String data,
        ...
        this.dsService.updateDataSourceAccount(puid, fo);
        this.auditService.logAndAddOperationAudit(puid, uid, request.getRequestURI(), request.getRemoteAddr(), fo
    ...
    public ResWebData<?> deleteAccount(@RequestBody @Valid DeleteAccountFO fo, HttpServletRequest request) {
        ...
        DmDsDO rdpDataSourceDO = this.dsService.queryById(fo.getDataSourceId());
        this.dsService.cleanDataSourceAccount(puid, fo.getDataSourceId());
        this.auditService.logAndAddOperationAudit(puid, uid, request.getRequestURI(), request.getRemoteAddr(), fo
