...
@Component
public class SchemaBehaviorCallBackHandler extends AbstractBehaviorCallBackHandler {

    @Override
    public void backfill(DmExecSqlAuditDO audit, List<BehaviorRelation> behaviors) {
        for (String schemaPath : affectedPaths(behaviors, TargetType.Schema, BehaviorAction.CREATE, BehaviorAction.ALTER, BehaviorAction.DROP)) {
            deleteCache(audit, parentPath(schemaPath), MetaInformationType.SchemaList);
        }
    }
}
