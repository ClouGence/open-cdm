...
public class WechatApi {
    ...
    public String getUserIdByPhone(final String phone) {
        try {
            ...
            if (e.getErrcode().equals(46004)) {
                throw ThirdPartyApiException.as().with(e, WechatI18nKeys.WECHAT_NOT_FIND_USER_BY_PHONE, phone);
            }
            throw e;
        }
    }
    ...
    public ApprovalDetail getInstanceDetail(final String instanceId) {
        try {
            ...
            if (e.getErrcode().equals(301026)) {
                throw ThirdPartyApiException.as().with(e, WechatI18nKeys.WECHAT_NOT_FIND_APPROVAL_INSTANCE, instanceId);
            }
            throw e;
        }
    }
    ...
    public ApprovalTmpDetailResponse getTemplate(final String code) {
        try {
            ...
            if (e.getErrcode().equals(301025)) {
                throw ThirdPartyApiException.as().with(e, ThirdPartyApiErrorType.APPROVAL_TEMPLATE_NOT_EXISTS, WechatI18nKeys.WECHAT_APPROVAL_TEMPLATE_NOT_EXISTS, code);
            }
            throw e;
        }
    }
    ...
    public String getUserTicket(String authCode) {
        try {
            ...
            if (!response.isSuccessful()) {
                throw ThirdPartyApiException.as().with(WechatI18nKeys.WECHAT_API_USERTICKET_ERROR, response.code() + ":" + response.body().string());
            }
            JSONObject object = JSONObject.parseObject(response.body().string());
            int errcode = object.getIntValue("errcode");
            if (errcode != 0) {
                throw ThirdPartyApiException.as().with(WechatI18nKeys.WECHAT_API_USERTICKET_ERROR, object.getString("errmsg"));
            }
            ...
            if (StringUtils.isNotBlank(userTicket)) {
                ...
                throw ThirdPartyApiException.as().with(WechatI18nKeys.WECHAT_IS_OPEN_USER_ERROR);
            }
        } catch (ThirdPartyApiException e) {
            throw e;
        } catch (Exception e) {
            log.error("wechat getUserinfo failed, " + e.getMessage());
            throw ThirdPartyApiException.as().with(e, WechatI18nKeys.WECHAT_API_USERTICKET_ERROR, e.getMessage());
        }
    }
    ...
    public UserData getUserInfo(String userTicket) {
        try {
            ...
            Response response = httpClient.newCall(request).execute();
            if (!response.isSuccessful()) {
                throw ThirdPartyApiException.as().with(WechatI18nKeys.WECHAT_GETUSERINFO_ERROR, response.code() + ":" + response.body().string());
            }

            JSONObject object = JSONObject.parseObject(response.body().string());
            int errcode = object.getIntValue("errcode");
            if (errcode != 0) {
                throw ThirdPartyApiException.as().with(WechatI18nKeys.WECHAT_GETUSERINFO_ERROR, object.getString("errmsg"));
            }
            ...
            log.error("wechat getUserInfo failed, " + e.getMessage());
            throw ThirdPartyApiException.as().with(e, WechatI18nKeys.WECHAT_GETUSERINFO_ERROR, e.getMessage());
        }
    }
}
