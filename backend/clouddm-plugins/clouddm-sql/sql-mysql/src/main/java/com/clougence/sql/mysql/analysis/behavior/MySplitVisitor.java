...
public class MySplitVisitor extends MySqlParserBaseVisitor<StatementType> {
    ...
    private boolean                     performanceTableRead;
    ...
    public Set<StatementType> collectTypes(ParseTree tree) {
        ...
        this.performanceTableRead = false;
        ...
        if (this.performanceTableRead) {
            this.types.add(StatementType.PERFORMANCE);
            if (!this.ordinaryTableRead && !this.metadataTableRead) {
                this.types.remove(StatementType.SELECT);
            }
        }
    ...
    private void collectTableRead(AtomTableItemContext ctx) {
        ...
        String schema = normalizeIdentifier(fullId.uid(0).getText());
        String object = fullId.identifierAfterDot != null ? fullId.identifierAfterDot.getText() : fullId.uid(fullId.uid().size() - 1).getText();
        if ("performance_schema".equals(schema)) {
            this.performanceTableRead = true;
        } else if ("information_schema".equals(schema) || resources.isMetadataTable(schema, object, this.version)) {
    ...
    private StatementType selectType(ParseTree tree) {
        ...
        StatementType functionAction = preferredFunctionAction(tree);
        if (isManagementFunctionAction(functionAction)) {
            return StatementType.SELECT;
        }
        if (functionAction == StatementType.PERFORMANCE) {
            return StatementType.SELECT;
        }
        if (functionAction == StatementType.LOG_READ) {
    ...
    private boolean isUserDefinedFunction(GenericFunctionCallContext ctx) {
        ...
        if (fullId == null) {
            return resources.isUserDefinedFunction(custom.function.getText(), false, this.version);
        }
