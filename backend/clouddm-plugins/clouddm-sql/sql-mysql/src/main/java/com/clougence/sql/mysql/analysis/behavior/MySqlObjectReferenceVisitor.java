...
public class MySqlObjectReferenceVisitor extends MySqlParserBaseVisitor<Void> {
    ...
    public Void visitGenericFunctionCall(GenericFunctionCallContext ctx) {
        if (ctx.genericFunction().name instanceof CustomGenericFunctionNameContext custom) {
            ...
            String functionName = custom.function.getText();
            boolean qualified = false;
            ParserRuleContext reference = custom.function;
            if (fullId != null) {
                functionName = fullId.identifierAfterDot != null ? fullId.identifierAfterDot.getText() : fullId.uid(fullId.uid().size() - 1).getText();
                qualified = fullId.DOT() != null;
                reference = fullId;
            }
            if (resources.isUserDefinedFunction(functionName, qualified, this.version)) {
                add(StatementType.CALL_PROG_OBJ, TargetType.Function, reference);
            }
