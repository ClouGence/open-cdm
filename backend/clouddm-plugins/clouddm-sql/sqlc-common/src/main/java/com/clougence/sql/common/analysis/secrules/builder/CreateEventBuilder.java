...
import com.clougence.clouddm.sdk.sql.analysis.security.rdb.RdbEventDomain;
...
public class CreateEventBuilder extends AbstractDomainBuilder {

    private final RdbEventDomain domain = new RdbEventDomain();

    @Override
    public List<Domain> build() {
        domain.setSqlType(RuleQueryType.CREATE_EVENT);
    ...
    public void handleSubDomain(List<Domain> list, DomainSource source) {
        if (source == DomainSource.OBJ_NAME) {
            ...
            domain.setName(map.get(UmiTypes.Function));
        } else {
