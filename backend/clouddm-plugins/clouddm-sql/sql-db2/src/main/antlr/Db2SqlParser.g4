...
insert_statement
    ...
    (column_name_list_paren | column_name_list)? include_columns?
...
alter_table_opts
    ...
    | ADD INDEX index_name '(' index_col_opts ')'
    | RENAME TO target_identifier
    | RENAME COLUMN? s=column_name TO t=column_name
    | DROP ( PRIMARY KEY
...
element_list_item
    ...
    | referential_constraint
    | check_constraint
    | INDEX index_name '(' index_col_opts ')'
    ;
...
singles_table_reference
    : table_name period_specification* correlation_clause? tablesample_clause? join_clause*
    ;

join_clause
    : (INNER | outer)? JOIN singles_table_reference (ON join_condition | USING '(' column_name_list ')')?
    | CROSS JOIN singles_table_reference
    ;
