 ...
 */
package com.clougence.sql.db2.security.resolve;
...
import com.clougence.sql.db2.security.domain.Db2ColumnDomain;
import com.clougence.sql.db2.security.domain.Db2SelectDomain;
import com.clougence.sql.db2.security.domain.Db2TableDomain;
import com.clougence.utils.StringUtils;

public abstract class Db2AbstractResolveHelper implements Db2SecDomainOptionKeys {
    ...
    protected void rewriteType(Db2ColumnDomain columnDomain, SQLDataType dataType) {
    ...
    protected Db2ColumnDomain toColumnDomain(Db2TableDomain tableDomain, SQLColumnDefinition columnDef, SQLDataType inDataType) {
        SQLDataType dataType = inDataType != null ? inDataType : columnDef.getDataType();
    ...
    protected RdbCallDomain toCallDomain(SQLMethodInvokeExpr invokeExpr) {
        return toCallDomain(invokeExpr.getMethodName(), invokeExpr.getOwner() == null ? null : invokeExpr.getOwner().toString());
    }

    protected RdbCallDomain toCallDomain(String methodName, String schemaName) {
        ...
        callDomain.setSchema(schemaName);
    ...
    protected Db2SelectDomain newQueryDomain(RdbQueryMode mode) {
    ...
    protected MetaTable findOwnerTable(MetaCollectTables ctxTabs, SQLExpr target) {
        ...
        if (target instanceof SQLIdentifierExpr) {
            ...
            if (owner instanceof SQLIdentifierExpr) {
                ...
                throw new UnsupportedOperationException("unsupported findOwnerTable of " + target);
            }
    ...
    protected String extractTypeName(Db2ColumnDomain domain, SQLDataType dataType) {
