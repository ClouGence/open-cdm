...
public class Db2CallResolveHelper extends Db2AbstractResolveHelper {
    ...
    public List<RuleDomain> fromCall(SQLCallStatement statement) {
        ...
        if (sqlName instanceof SQLPropertyExpr expr) {
            domain.setCallName(expr.getName());
            domain.setSchema(expr.getOwnerName());
        } else if (sqlName instanceof SQLIdentifierExpr expr) {
            domain.setCallName(expr.getName());
        }
