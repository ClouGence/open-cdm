 ...
 */
package com.clougence.sql.db2.security;
...
import com.clougence.sql.db2.security.resolve.*;
...
public class Db2SecDomainResolveSpi implements SecDomainResolveSpi, Db2SecDomainOptionKeys {
    ...
    private List<RuleDomain> parseSql(SQLStatement statement) {
        ...
        if (statement instanceof SQLCreateViewStatement createViewStatement) {
            SQLExprTableSource tableSource = createViewStatement.getTableSource();
            if (tableSource.getExpr() instanceof SQLIdentifierExpr identifierExpr) {
