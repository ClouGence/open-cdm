...
public class MsSqlParserVisitor extends SqlServerParserBaseVisitor<Void> {
    ...
    public Void visitCreate_database(SqlServerParser.Create_databaseContext ctx) {
        RdbCatalogDomain domain = builder.newCatalogDomain(SecQueryType.CREATE_CATALOG);
    ...
    public Void visitDrop_database(SqlServerParser.Drop_databaseContext ctx) {
        for (SqlServerParser.Id_Context id : ctx.id_()) {
            RdbCatalogDomain domain = builder.newCatalogDomain(SecQueryType.DROP_CATALOG);
    ...
    public Void visitCreate_schema(SqlServerParser.Create_schemaContext ctx) {
        ...
        if (!names.isEmpty()) {
            MsSchemaDomain domain = builder.newSchemaDomain(SecQueryType.CREATE_SCHEMA);
    ...
    public Void visitDrop_schema(SqlServerParser.Drop_schemaContext ctx) {
        List<String> names = names(ctx);
        if (!names.isEmpty()) {
            MsSchemaDomain domain = builder.newSchemaDomain(SecQueryType.DROP_SCHEMA);
    ...
    public Void visitCreate_table(SqlServerParser.Create_tableContext ctx) {
        MsTableDomain domain = tableDomain(ctx.table_name(), SecQueryType.CREATE_TABLE, SecQueryKind.CREATE);
        domain.setColumns(new ArrayList<>());
        Map<String, MsColumnDomain> columns = new LinkedHashMap<>();
        List<RuleDomain> children = tableElementDomains(ctx.column_def_table_constraints(), domain, columns, SecQueryType.CREATE_TABLE, SecQueryType.CREATE_TABLE);
        add(domain);
    ...
    public Void visitAlter_table(SqlServerParser.Alter_tableContext ctx) {
        MsTableDomain domain = tableDomain(ctx.table_name(0), SecQueryType.ALTER_TABLE, SecQueryKind.ALTER);
        domain.setColumns(new ArrayList<>());
        Map<String, MsColumnDomain> columns = new LinkedHashMap<>();
        List<RuleDomain> children = new ArrayList<>();
        if (ctx.ADD() != null && ctx.column_def_table_constraints() != null) {
            children.addAll(tableElementDomains(ctx.column_def_table_constraints(), domain, columns, SecQueryType.ADD_COLUMN, SecQueryType.ADD_CONSTRAINT));
        } else if (ctx.ALTER().size() > 1 && ctx.column_definition() != null) {
            MsColumnDomain column = columnDomain(ctx.column_definition(), domain, SecQueryType.ALTER_COLUMN, SecQueryKind.ALTER);
            children.add(column);
            domain.getColumns().add(column.getColumn());
        } else if (ctx.DROP() != null && ctx.COLUMN() != null) {
            for (SqlServerParser.Id_Context id : ctx.id_()) {
                MsColumnDomain column = simpleColumnDomain(domain, clean(id), SecQueryType.DROP_COLUMN, SecQueryKind.DROP);
                children.add(column);
                domain.getColumns().remove(column.getColumn());
            }
        } else if (ctx.DROP() != null && ctx.CONSTRAINT() != null && ctx.constraint != null) {
            RdbConstraintDomain constraint = constraintDomain(domain, SecQueryType.DROP_CONSTRAINT, SqlConstraintType.ByName, clean(ctx.constraint), Collections.emptyList());
    ...
    public Void visitDrop_table(SqlServerParser.Drop_tableContext ctx) {
        for (SqlServerParser.Table_nameContext tableName : ctx.table_name()) {
            MsTableDomain domain = tableDomain(tableName, SecQueryType.DROP_TABLE, SecQueryKind.DROP);
            domain.setIfExists(ctx.IF() != null);
            add(domain);
        }
        return null;
    ...
    public Void visitCreate_view(SqlServerParser.Create_viewContext ctx) {
        ...
        RdbViewDomain domain = builder.newViewDomain(alter ? SecQueryType.ALTER_VIEW : SecQueryType.CREATE_VIEW);
    ...
    public Void visitDrop_view(SqlServerParser.Drop_viewContext ctx) {
        for (SqlServerParser.Simple_nameContext simpleName : ctx.simple_name()) {
            ...
            RdbViewDomain domain = builder.newViewDomain(SecQueryType.DROP_VIEW);
            domain.setCatalog(name.catalog);
            domain.setSchema(name.schema);
            domain.setView(name.name);
            add(domain);
    ...
    public Void visitSelect_statement_standalone(SqlServerParser.Select_statement_standaloneContext ctx) {
        ...
        for (SqlServerParser.Table_source_itemContext source : descendants(ctx, SqlServerParser.Table_source_itemContext.class)) {
            ...
            MsTableDomain tableDomain = tableDomain(tableName, SecQueryType.SELECT, SecQueryKind.QUERY);
    ...
    public Void visitUse_statement(SqlServerParser.Use_statementContext ctx) {
        RdbCatalogDomain domain = builder.newCatalogDomain(SecQueryType.SWITCH_CATALOG);
        domain.setCatalog(clean(ctx.database));
        add(domain);
        return null;
    }
    ...
    public Void visitCreate_sequence(SqlServerParser.Create_sequenceContext ctx) {
        ...
        RdbSequenceDomain domain = builder.newSequenceDomain(SecQueryType.CREATE_SEQUENCE);
        domain.setSchema(name.schema);
    ...
    public Void visitCreate_synonym(SqlServerParser.Create_synonymContext ctx) {
        RdbResourceDomain domain = builder.newResourceDomain(SecQueryType.CREATE_SYNONYM, SecQueryKind.CREATE);
    ...
    public Void visitCreate_login_sql_server(SqlServerParser.Create_login_sql_serverContext ctx) {
        RdbUserDomain domain = builder.newUserDomain(SecQueryType.CREATE_USER);
    ...
    public Void visitCreate_or_alter_function(SqlServerParser.Create_or_alter_functionContext ctx) {
        ...
        boolean alter = startsWithAlter(ctx);
        RdbFunctionDomain domain = builder.newFunctionDomain(alter ? SecQueryType.ALTER_PROG_OBJ : SecQueryType.CREATE_PROG_OBJ);
        domain.setCatalog(name.catalog);
        domain.setSchema(name.schema);
        domain.setName(name.name);
        add(domain);
    ...
    public Void visitCreate_or_alter_procedure(SqlServerParser.Create_or_alter_procedureContext ctx) {
        NameParts name = parts(ctx.func_proc_name_schema());
        boolean alter = startsWithAlter(ctx);
        RdbProcedureDomain domain = builder.newProcedureDomain(alter ? SecQueryType.ALTER_PROG_OBJ : SecQueryType.CREATE_PROG_OBJ);
        domain.setCatalog(name.catalog);
        domain.setSchema(name.schema);
        domain.setName(name.name);
        add(domain);
    ...
    public Void visitCreate_or_alter_dml_trigger(SqlServerParser.Create_or_alter_dml_triggerContext ctx) {
        ...
        NameParts table = parts(ctx.table_name());
        boolean alter = startsWithAlter(ctx);
        RdbTriggerDomain domain = builder.newTriggerDomain(alter ? SecQueryType.ALTER_TRIGGER : SecQueryType.CREATE_TRIGGER);
        domain.setCatalog(table.catalog);
        domain.setSchema(table.schema);
        domain.setTable(table.name);
    ...
    private MsTableDomain tableDomain(ParserRuleContext ctx, SecQueryType type, SecQueryKind kind) {
        NameParts name = parts(ctx);
    ...
    private List<RuleDomain> tableElementDomains(SqlServerParser.Column_def_table_constraintsContext ctx, MsTableDomain tableDomain, Map<String, MsColumnDomain> columns,
                                                 SecQueryType columnType, SecQueryType constraintType) {
        if (ctx == null) {
    ...
    private MsColumnDomain columnDomain(SqlServerParser.Column_definitionContext ctx, MsTableDomain tableDomain, SecQueryType type, SecQueryKind kind) {
    ...
    private MsColumnDomain simpleColumnDomain(MsTableDomain tableDomain, String column, SecQueryType type, SecQueryKind kind) {
    ...
    private RdbConstraintDomain tableConstraintDomain(SqlServerParser.Table_constraintContext ctx, MsTableDomain tableDomain, SecQueryType type) {
    ...
    private RdbConstraintDomain alterTableConstraintDomain(SqlServerParser.Alter_tableContext ctx, MsTableDomain tableDomain) {
        ...
        RdbConstraintDomain domain = constraintDomain(tableDomain, SecQueryType.ADD_CONSTRAINT, type, clean(ctx.constraint), columns);
        markTableConstraint(tableDomain, Collections.emptyMap(), domain);
        return domain;
    }

    private RdbConstraintDomain constraintDomain(MsTableDomain tableDomain, SecQueryType queryType, SqlConstraintType constraintType, String name, List<String> columns) {
    ...
    private boolean tryAddRenameDomain(SqlServerParser.Execute_statementContext ctx) {
        ...
        if (target.contains("DATABASE")) {
            MsCatalogDomain domain = builder.newCatalogDomain(SecQueryType.RENAME_CATALOG);
        ...
        if (target.contains("COLUMN")) {
            MsColumnDomain domain = builder.newColumnDomain(SecQueryType.RENAME_COLUMN, SecQueryKind.ALTER);
        ...
        if (target.contains("OBJECT")) {
            MsTableDomain domain = builder.newTableDomain(SecQueryType.RENAME_TABLE, SecQueryKind.ALTER);
