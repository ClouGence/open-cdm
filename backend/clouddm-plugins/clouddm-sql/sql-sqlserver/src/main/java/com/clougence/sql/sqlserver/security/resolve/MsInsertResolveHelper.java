...
import com.clougence.sql.sqlserver.security.domain.MsInsertDomain;
import com.clougence.utils.CollectionUtils;

public class MsInsertResolveHelper extends MsQueryResolveHelper {
    ...
    public List<RuleDomain> fromInsert(SQLServerInsertStatement statement) {
    ...
    private void fromInsertQuery(List<RuleDomain> result, MetaCollectTables ctxTabs, MsInsertDomain domain, SQLSelect query) {
        if (query == null) {
            return;
        }
    ...
    private void fromInsertValues(MsInsertDomain domain, MetaCollectTables ctxTabs, SQLServerInsertStatement statement, List<RuleDomain> result) {
        ...
        for (SQLInsertStatement.ValuesClause values : valuesList) {
            for (SQLExpr value : values.getValues()) {
                if (value instanceof SQLQueryExpr queryExpr) {
    ...
    private void fromInsertColumns(MsInsertDomain domain, SQLServerInsertStatement statement) {
        ...
        for (SQLExpr column : columns) {
            ...
            } else if (column instanceof SQLCharExpr) {
                // avoid  execute script
                /*
