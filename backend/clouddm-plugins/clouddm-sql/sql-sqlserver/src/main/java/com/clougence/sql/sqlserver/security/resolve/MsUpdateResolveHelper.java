...
public class MsUpdateResolveHelper extends MsQueryResolveHelper {

    protected MsUpdateDomain newUpdateDomain() {
    ...
    public List<RuleDomain> fromUpdate(SQLServerUpdateStatement statement) {
        List<RuleDomain> result = new ArrayList<>();
        MsUpdateDomain domain = newUpdateDomain();
    ...
    private void fromUpdate(List<RuleDomain> result, MetaCollectTables ctxTabs, MsUpdateDomain mainDomain, //
    ...
    private void fromUpdateSet(List<RuleDomain> result, MetaCollectTables ctxTabs, MsUpdateDomain domain, List<SQLUpdateSetItem> sets) {
        domain.setSetColumns(new ArrayList<>());
        for (SQLUpdateSetItem item : sets) {
            SQLExpr column = item.getColumn();
            if (column instanceof SQLIdentifierExpr expr) {
                domain.getSetColumns().add(getColumnNameRemoveQualifier(expr.getName()));
            } else if (column instanceof SQLPropertyExpr expr) {
                domain.getSetColumns().add(getColumnNameRemoveQualifier(expr.getName()));
            }
