 ...
 */
package com.clougence.sql.sqlserver.security.resolve;
...
import com.clougence.clouddm.sdk.sql.secrules.ref.MetaCollectTables;
import com.clougence.clouddm.sdk.sql.secrules.ref.MetaTable;
import com.clougence.sql.sqlserver.security.domain.MsSelectDomain;
import com.clougence.utils.StringUtils;

public class MsQueryResolveHelper extends MsAbstractResolveHelper {

    protected MsCollectTableHelper helper;

    public MsQueryResolveHelper(){
        this.helper = new MsCollectTableHelper();
    }
    ...
    protected void fromSelect(List<RuleDomain> result, MetaCollectTables ctxTabs, SQLSelect select, RdbQueryMode mode) {
        ...
        MsSelectDomain domain = newQueryDomain(mode);
        result.add(domain);
    ...
    protected void fromSelectForUnion(List<RuleDomain> result, MetaCollectTables ctxTabs, SQLSelectQuery select) {
        ...
        MsSelectDomain domain = newQueryDomain(RdbQueryMode.UNION);
        result.add(domain);
    ...
    protected void fromSelectForSourceIsSub(List<RuleDomain> result, MetaCollectTables ctxTabs, SQLSelect select, String alias) {
        MsSelectDomain domain = newQueryDomain(RdbQueryMode.SUB_FROM);
    ...
    protected void fromSelectTestSimpleQuery(MsSelectDomain domain, SQLSelectQuery query) {
    ...
    protected void fromSelectMainQuery(List<RuleDomain> result, MetaCollectTables ctxTabs, MsSelectDomain domain, SQLSelectQuery query) {
        if (query instanceof SQLUnionQuery) {
            ...
            domain.setHasLimit(((SQLServerSelectQueryBlock) query).getTop() != null);
    ...
    protected void fromSelectSelect(List<RuleDomain> result, MetaCollectTables ctxTabs, MsSelectDomain domain, List<SQLSelectItem> items) {
    ...
    protected void fromSelectSelectArgs(List<RuleDomain> result, MetaCollectTables ctxTabs, MsSelectDomain domain, SQLExpr sqlExpr) {
        if (sqlExpr instanceof SQLBinaryOpExpr) {
            ...
            markFuncInSelect(domain);
            RdbCallDomain callDomain = toCallDomain("CAST");
            callDomain.setFunc(true);
    ...
    protected void fromSelectFrom(List<RuleDomain> result, MetaCollectTables ctxTabs, MsSelectDomain domain, SQLTableSource source, SQLExpr whereExpr) {
        if (source == null) {
        ...
        if (source instanceof SQLExprTableSource) {
            ...
            // sub from
            MsSelectDomain subFormDomain = newQueryDomain(RdbQueryMode.SUB_FROM);
            ...
            throw new UnsupportedOperationException("unsupported SQL: " + source);
        }
    }
    ...
    protected void fromWithQuery(List<RuleDomain> result, MetaCollectTables ctxTabs, RuleDomain domain, SQLWithSubqueryClause withQuery) {
        ...
        for (SQLWithSubqueryClause.Entry entry : withQuery.getEntries()) {
            ...
            MsSelectDomain withDomain = newQueryDomain(RdbQueryMode.WITH);
    ...
    protected void fromWhereExprItem(List<RuleDomain> result, MetaCollectTables ctxTabs, RdbWhereDomain domain, SQLExpr itemExpr) {
        ...
        if (hasColName) {
            ...
            if (testBelong(domain, findOwnerTable(ctxTabs, columnName))) {
                ...
                } else if (columnName instanceof SQLInListExpr) {
                    domain.addWhereColumn(getString(((SQLInListExpr) columnName).getExpr()));
                }
            }
        }
    ...
    protected void markHasWith(RuleDomain domain) {
        if (domain instanceof MsSelectDomain) {
            ((MsSelectDomain) domain).setHasWith(true);
            ((MsSelectDomain) domain).setSimpleSelect(false);
    ...
    protected void markHasAs(RuleDomain domain) {
        if (domain instanceof MsSelectDomain) {
            ((MsSelectDomain) domain).setSimpleSelect(false);
            ((MsSelectDomain) domain).setHasAs(true);
        }
    }

    protected void markHasUnion(RuleDomain domain) {
        if (domain instanceof MsSelectDomain) {
            ((MsSelectDomain) domain).setHasUnion(true);
            ((MsSelectDomain) domain).setSimpleSelect(false);
        } else if (domain instanceof RdbWhereDomain) {
