...
public class GpConfig extends DataSourceConfig {
    ...
    @ConfigDef(name = Fields.connectTimeoutMs, defaultValue = "5000", //
            group = DsConfigGroup.ADVANCED, labelKey = GpConfigI18nKeys.CONFIG_RDB_CONN_TIMEOUT_MS_LABEL, descKey = GpConfigI18nKeys.CONFIG_RDB_CONN_TIMEOUT_MS_DESC, readOnly = false)
    private Long    connectTimeoutMs;
    @ConfigDef(name = Fields.soTimeoutSec, defaultValue = "10", //
            group = DsConfigGroup.ADVANCED, labelKey = GpConfigI18nKeys.CONFIG_DS_SO_TIMEOUT_MS_LABEL, descKey = GpConfigI18nKeys.CONFIG_DS_SO_TIMEOUT_MS_DESC, readOnly = false)
    ...
    public Properties asDriverProperties() {
        ...
        if (this.getSslMode() == SslMode.CA && !hasSslCaConfig()) {
            throw new IllegalArgumentException("Greenplum CA certificate is required.");
        }
        if (this.getSslMode() == SslMode.CLIENT_CERT) {
            ...
            if (!hasSslClientCertConfig()) {
                throw new IllegalArgumentException("Greenplum client certificate is required.");
            }
            if (!hasSslClientKeyConfig()) {
                throw new IllegalArgumentException("Greenplum client private key is required.");
            }
        }
