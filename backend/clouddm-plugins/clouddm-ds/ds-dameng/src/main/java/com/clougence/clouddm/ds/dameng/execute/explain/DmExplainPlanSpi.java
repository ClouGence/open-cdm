 ...
 */
package com.clougence.clouddm.ds.dameng.execute.explain;
...
/** Parses Dameng EXPLAIN FOR rows. */
public class DmExplainPlanSpi implements ExplainPlanSpi {
    ...
    public ExplainPlan analyze(List<Result> results, List<BehaviorRelation> relations) {
        ...
        for (Result result : results) {
            ...
            for (ResultSetRow row : resultSet.getRowSet()) {
                ...
                node.setNodeId(value(row, indexes.get("lineid")));
                node.setLogical(value(row, indexes.get("operation")));
                node.setPhysical(value(row, indexes.get("scan_type")));
                node.setObjectPath(value(row, indexes.get("tab_name")));
                node.setEstimatedRows(number(value(row, indexes.get("row_nums"))));
