 ...
 */
package com.clougence.clouddm.ds.sqlserver.execute.explain;
...
/** Parses SQL Server SET SHOWPLAN_ALL output. */
public class MsSqlExplainPlanSpi implements ExplainPlanSpi {
    ...
    public ExplainPlan analyze(List<Result> results, List<BehaviorRelation> relations) {
        ...
        for (Result result : results) {
            ...
            for (ResultSetRow row : resultSet.getRowSet()) {
                ...
                node.setNodeId(value(row, indexes.get("nodeid")));
                node.setLogical(value(row, indexes.get("logicalop")));
                node.setPhysical(value(row, indexes.get("physicalop")));
                node.setEstimatedRows(number(value(row, indexes.get("estimaterows"))));
