...
import com.clougence.sql.redis.parser.ast.commands.keys.CopyRedisCmd;
...
@Slf4j
class GlideCallForDB extends GlideUtils {

    public static CgFuture<?> cmdCopyRedisCmd(CgFuture<Object> sync, GlideCmd glideCmd, CopyRedisCmd redisCmd, AdapterRequest request, AdapterReceive receive) throws SQLException {
        ...
        if (redisCmd.getDstDB() != null) {
            ...
            result = glideCmd.copy(oldKeyName, newKeyName, destDataBase, replace);
        } else {
            result = glideCmd.copy(oldKeyName, newKeyName);
        }
