...
import com.clougence.clouddm.ds.valkey.definition.ui.editor.keys.ValkeyEditorProvider;
import com.clougence.clouddm.ds.valkey.execute.jdbc.GlideConnection;
...
public class ValkeyMetaService extends DefaultRdbMetaService {
    ...
    public ValkeyMetaService(Session rdbSession){
        super(rdbSession);
    }

    @Override
    protected DmRdbUmiService rdbUmiService(Connection con) {
        return new ValkeyUmiServiceDm(con);
    }

    @Override
    protected SqlBuilder getSqlBuilder() { return ValkeyEditorProvider.INSTANCE; }
    ...
    public String getCurrentSchema() {
        try {
            return this.rdbSession.executeQuery(con -> con.unwrap(GlideConnection.class).getSchema());
        } catch (Exception e) {
            ...
            this.rdbSession.executeQuery(con -> {
                try (PreparedStatement ps = con.prepareStatement("ping ?;")) {
                    ...
                    try (ResultSet rs = ps.executeQuery()) {
                        ...
                        throw new SQLException("valkey ping test failed.");
                    }
                }
            });
        } catch (Exception e) {
