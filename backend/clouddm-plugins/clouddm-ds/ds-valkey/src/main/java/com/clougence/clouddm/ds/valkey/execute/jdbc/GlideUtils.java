...
class GlideUtils {
    ...
    protected static AdapterResultCursor listResult(AdapterRequest request, Map<String, Double> result) throws SQLException {
        ...
        for (Map.Entry<String, Double> entry : result.entrySet()) {
            receiveCur.pushData(CollectionUtils.asMap(          //
                    COL_SCORE_DOUBLE.name, entry.getValue(),    //
                    COL_ELEMENT_STRING.name, entry.getKey()     //
            ));
            affectRows++;
    ...
    protected static ExpireOptions convertToExpiryOption(KeyOptToken.KeyOptionType opt) throws SQLException {
        switch (opt) {
            case NX:
    ...
    protected static ScoreRange parseScoreRange(String token) {
        if (token == null) {
            return null;
        }
        if ("+inf".equalsIgnoreCase(token)) {
            return InfScoreBound.POSITIVE_INFINITY;
        }
        if ("-inf".equalsIgnoreCase(token)) {
            return InfScoreBound.NEGATIVE_INFINITY;
        }
        if (token.startsWith("(")) {
            return new ScoreBoundary(Double.parseDouble(token.substring(1)), true);
        }
        if (token.startsWith("[")) {
            return new ScoreBoundary(Double.parseDouble(token.substring(1)), false);
        }
        return new ScoreBoundary(Double.parseDouble(token), false);
    }

    protected static LexRange parseLexRange(String token) {
        if (token == null) {
            return null;
        }
        if ("+".equals(token)) {
        ...
        if (token.startsWith("(")) {
            return new LexBoundary(token.substring(1), true);
        }
        if (token.startsWith("[")) {
            return new LexBoundary(token.substring(1), false);
        }
        return new LexBoundary(token, false);
    }

    protected static Aggregate convertToAggregate(AggregateType aggregate) throws SQLException {
        switch (aggregate) {
            case MIN:
