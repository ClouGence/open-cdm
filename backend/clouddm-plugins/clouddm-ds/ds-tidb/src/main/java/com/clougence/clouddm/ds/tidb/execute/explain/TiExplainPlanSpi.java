 ...
 */
package com.clougence.clouddm.ds.tidb.execute.explain;
...
/** Parses TiDB's operator table, including its datasource-specific estRows column. */
public class TiExplainPlanSpi implements ExplainPlanSpi {
    ...
    public ExplainPlan analyze(List<Result> results, List<BehaviorRelation> relations) {
        ...
        for (Result result : results) {
            ...
            for (ResultSetRow row : resultSet.getRowSet()) {
                ...
                node.setLogical(value(row, indexes.get("id")));
                node.setPhysical(value(row, indexes.get("task")));
                node.setObjectPath(value(row, indexes.get("access object")));
                node.setEstimatedRows(number(value(row, indexes.get("estrows"))));
                node.setProperties(properties(row, columns));
                plan.getNodes().add(node);
            }
        }
        BehaviorRelation write = writeRelation(relations);
        if (write != null) {
            ...
            if (write.getInsertRows() != null) {
                ...
                target.setEstimatedRows(plan.getNodes().stream().map(ExplainPlanNode::getEstimatedRows).filter(rows -> rows != null).findFirst().orElse(null));
            }
        }
        setSource(plan, results, relations);
        return plan;
    }

    private static BehaviorRelation writeRelation(List<BehaviorRelation> relations) {
        if (relations == null) {
            return null;
        }
    ...
    private static void setSource(ExplainPlan plan, List<Result> results, List<BehaviorRelation> relations) {
