...
public abstract class TiDBParserBase extends Parser {
    ...
    protected TiDBParserBase(TokenStream input){
        super(input);
    }
    ...
    protected final boolean tidbAtLeastOrExecutableComment(int minimumMajor) {
        return tidbAtLeast(minimumMajor) || isInsideTiDBExecutableComment();
    }

    protected final boolean tidbAtMost(int maximumMajor) {
        return config.version().major() <= maximumMajor;
    }

    private boolean isInsideTiDBExecutableComment() {
        int currentIndex = getCurrentToken().getTokenIndex();
        for (int index = currentIndex - 1; index >= 0; index--) {
            int tokenType = getInputStream().get(index).getType();
            if (tokenType == TiDBParser.TIDB_EXEC_COMMENT_LEFT) {
                return true;
            }
            if (tokenType == TiDBParser.EXEC_COMMENT_RIGHT) {
                return false;
            }
        }
        return false;
    ...
    protected final boolean isSqlModeKnown() { return config.sqlModeKnown(); }

    protected final boolean isSetVariableAssignmentAllowed(TiDBParser.VariableClauseContext variable) {
        if (config.atMost(5, 7)) {
            return true;
        }
    ...
    protected final boolean isTruthPredicateAllowed(ParserRuleContext suffix) {
        ParserRuleContext comparison = suffix.getParent() instanceof ParserRuleContext ? suffix.getParent() : null;
        if (comparison == null || comparison.children == null) {
            return true;
        }
        return comparison.children.stream().noneMatch(child -> child != suffix && child instanceof TiDBParser.TruthPredicateContext);
    }

    protected final boolean isPipesAsConcat() { return config.isEnabled(TiDBParserFeature.PIPES_AS_CONCAT); }

    protected final boolean isHighNotPrecedence() { return config.isEnabled(TiDBParserFeature.HIGH_NOT_PRECEDENCE); }
    ...
    protected final boolean isSimpleIdentifierAllowed() {
        ...
        if (previous != null && previous.getType() == TiDBParser.DOT) {
            return true;
        }
        if (getInputStream().LT(1).getType() == TiDBParser.ID) {
            return true;
        }
    ...
    protected final boolean isColumnConstraintSequenceAllowed(List<TiDBParser.ColumnConstraintContext> constraints) {
        if (atLeast(8, 0) || constraints == null || constraints.isEmpty()) {
            return true;
        }

        boolean checkSeen = false;
        for (TiDBParser.ColumnConstraintContext constraint : constraints) {
            if (constraint instanceof TiDBParser.CheckColumnConstraintContext) {
                if (checkSeen) {
                    return false;
                }
                checkSeen = true;
    ...
    protected final boolean isTypedTemporalLiteralAhead() {
        ...
        return tokenType == TiDBParser.DATE || tokenType == TiDBParser.TIME || tokenType == TiDBParser.TIMESTAMP;
    }

    protected final boolean isBareCharsetIntroducerIdentifier() {
        int nextTokenType = getInputStream().LA(2);
        return nextTokenType != TiDBParser.STRING_LITERAL && nextTokenType != TiDBParser.DOUBLE_QUOTE_STRING_LITERAL
               && nextTokenType != TiDBParser.DOUBLE_QUOTE_AMBIGUOUS;
    }
    ...
    protected final boolean isGenericFunctionCallAllowed(String functionName) {
        ...
        return !SPECIAL_SYNTAX_FUNCTIONS.contains(normalized);
    }

    protected final boolean isGenericFunctionCallAllowed(ParserRuleContext function) {
        if (function != null && function.getStart().getType() == TiDBParser.ID) {
            return true;
        }
    ...
    protected final boolean isGenericFunctionCallAllowed(ParserRuleContext function, ParserRuleContext args) {
        if (function instanceof TiDBParser.ScalarGenericFunctionNameContext) {
            return isScalarFunctionCallAllowed(function.getText(), args) || isGenericFunctionCallAllowed(function.getText());
        }
        if (function != null && function.getText().indexOf('.') >= 0 && args instanceof TiDBParser.FunctionArgsContext) {
            for (TiDBParser.FunctionArgWithAliasContext argument : ((TiDBParser.FunctionArgsContext) args).functionArgWithAlias()) {
    ...
    protected final boolean isScalarFunctionNameAhead() {
        ...
        return getATN().nextTokens(getATN().ruleToStartState[TiDBParser.RULE_scalarFunctionName]).contains(tokenType);
    }
    ...
    protected final boolean isUnionAfterQuerySpecificationAllowed(TiDBParser.QuerySpecificationContext query) {
        return atMost(5, 7) || (query.orderByClause() == null && query.limitClause() == null);
    }

    protected final boolean isSelectTailIntoAllowed(TiDBParser.QuerySpecificationContext query) {
        return atLeast(8, 0) && (query == null || query.getRuleContexts(TiDBParser.SelectIntoExpressionContext.class).isEmpty());
    }
    ...
    protected final boolean isUnionAfterParenthesizedQueryAllowed(ParserRuleContext parenthesizedQuery) {
        ...
        if (!(parenthesizedQuery instanceof TiDBParser.ParenthesizedSelectContext parenthesized)) {
            return false;
        }
        TiDBParser.LegacyQueryExpressionContext legacy = parenthesized.legacyQueryExpression();
        ...
        TiDBParser.QuerySpecificationContext query = legacy.querySpecification();
    ...
    protected final boolean isScalarFunctionCallAllowed(String functionName, ParserRuleContext args) {
        ...
        // TiDB's parser accepts these zero-argument forms and leaves arity
        // validation to later semantic stages.
        if (argCount == 0 && Set.of("ASCII", "DATE", "FORMAT", "HOUR", "INSERT", "TRUNCATE", "WEEK").contains(normalized)) {
            return true;
        }
        switch (normalized) {
            case "DATE":
