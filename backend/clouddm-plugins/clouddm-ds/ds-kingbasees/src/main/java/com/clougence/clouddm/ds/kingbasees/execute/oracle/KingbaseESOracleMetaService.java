...
public class KingbaseESOracleMetaService extends OraMetaService {

    public KingbaseESOracleMetaService(Session session){
        super(session);
    }

    @Override
    protected DmRdbUmiService rdbUmiService(Connection connection) {
        return new KingbaseESOracleSchemaUmiServiceDm(connection);
    }
