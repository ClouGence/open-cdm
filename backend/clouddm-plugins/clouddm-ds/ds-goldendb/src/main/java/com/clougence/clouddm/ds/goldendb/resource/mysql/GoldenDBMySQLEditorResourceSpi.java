 ...
 */
package com.clougence.clouddm.ds.goldendb.resource.mysql;
...
public class GoldenDBMySQLEditorResourceSpi implements ResourceSpi {

    private final ResourceLoader loader;

    public GoldenDBMySQLEditorResourceSpi(ClassLoader loader){
        this.loader = new ClassPathResourceLoader(Objects.requireNonNull(loader, "loader"), "");
    }

    @Override
    public String name() {
        return DataSourceType.GoldenDBMySQL.name();
    }

    @Override
    public ResourceObject findResource(String category, String resourcePath, ResourceRequest request) {
        if (!ResourceCategory.EDITOR.getCode().equals(category) || !"keywords".equals(resourcePath)) {
            return null;
        }
        return new ClasspathResourceObject(loader, "META-INF/clougence/db-keywords/goldendb.keywords", "text/plain;charset=UTF-8", true, true);
    }
}
