 ...
 */
public abstract class AbstractMsSqlSessionSpi extends RdbSessionSpi {
    ...
    public SessionContextDTO createSessionContext(DataSourceConfig dsConfig, Map<String, Object> params) {
        ...
        String defaultCatalog = defaultCatalog(dsConfig);
        if (StringUtils.isBlank((String) params.get(PARAMS_DEFAULT_DB)) && StringUtils.isNotBlank(defaultCatalog)) {
            contextDTO.setRdbCatalog(defaultCatalog);
        }
    ...
    protected abstract String defaultCatalog(DataSourceConfig dsConfig);
}
