...
public class OraMetaService extends DefaultRdbMetaService {
    ...
    public Map<String, String> getSqlParserParameters() {
        String databaseVersion = this.fetchVersion("SELECT VERSION FROM PRODUCT_COMPONENT_VERSION WHERE PRODUCT LIKE 'Oracle%' AND ROWNUM = 1");
    ...
    private boolean excludeOraMaintainedSchemas() {
        try {
            Object value = this.rdbSession.getDsConfig().getClass().getMethod("getExcludeOraMaintainedSchemas").invoke(this.rdbSession.getDsConfig());
