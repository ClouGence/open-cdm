...
import com.clougence.clouddm.ds.cloudberry.sql.parser.antlr.CbSqlParserBaseVisitor;
import com.clougence.clouddm.sdk.sql.analysis.behavior.*;
import com.clougence.clouddm.sdk.sql.parser.SplitQueryType;
import com.clougence.schema.umi.struts.UmiTypes;
import com.clougence.sql.common.analysis.behavior.RdbBehaviorObjectFactory;

final class CbStatementBehaviorVisitor extends CbSqlParserBaseVisitor<Void> {

    private final RdbBehaviorObjectFactory objects;
    private final StatementBehavior        behavior = new StatementBehavior();

    CbStatementBehaviorVisitor(SplitQueryType statementType, Map<UmiTypes, Object> levels, int baseLine, int baseColumn){
    ...
    @Override
    public Void visitProtocol_privilege_stmt(CbSqlParser.Protocol_privilege_stmtContext ctx) {
    ...
    @Override
    public Void visitRole_stmt(CbSqlParser.Role_stmtContext ctx) {
        ...
        for (var node : Trees.getDescendants(ctx)) {
            if (!(node instanceof CbSqlParser.Cloudberry_role_optionContext option) || option.resource_kind() == null || option.role_name() == null) {
                continue;
            }
    ...
    private void addCreateRoleMemberships(CbSqlParser.Role_stmtContext ctx, BehaviorObject createdRole) {
        List<BehaviorObject> grantees = new ArrayList<>();
        for (CbSqlParser.Create_role_optionContext option : ctx.create_role_option()) {
            CbSqlParser.Role_name_listContext names = option.role_name_list();
            ...
            if (option.IN_P() != null) {
                for (CbSqlParser.Role_nameContext role : names.role_name()) {
    ...
    @Override
    public Void visitProtocol_stmt(CbSqlParser.Protocol_stmtContext ctx) {
        ...
        if (ctx.CREATE() != null) {
            ...
            for (CbSqlParser.Protocol_handlerContext handler : ctx.protocol_handler()) {
                add(BehaviorAction.CALL, this.objects
                    .object(TargetType.Function, handler.StringConstant().getSymbol(), CbProtocolHandlerName.parse(handler.StringConstant().getText())));
            }
    ...
    @Override
    public Void visitExternal_stmt(CbSqlParser.External_stmtContext ctx) {
    ...
    @Override
    public Void visitResource_stmt(CbSqlParser.Resource_stmtContext ctx) {
    ...
    @Override
    public Void visitRetrieve_stmt(CbSqlParser.Retrieve_stmtContext ctx) {
        add(BehaviorAction.READ, object(TargetType.Endpoint, ctx.identifier()));
        return null;
    }

    private BehaviorObject object(TargetType type, CbSqlParser.IdentifierContext identifier) {
        return this.objects.object(type, identifier, List.of(name(identifier.getText())));
    }

    private BehaviorObject object(TargetType type, CbSqlParser.Qualified_nameContext qualifiedName) {
        List<String> names = qualifiedName.identifier().stream().map(ParserRuleContext::getText).map(CbStatementBehaviorVisitor::name).toList();
