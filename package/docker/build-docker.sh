...
stage_build_context() {
  ...
  if [ -d "$PACKAGE_BUILD_DIR/built-in-drivers" ]; then
    cp -R "$PACKAGE_BUILD_DIR/built-in-drivers/." "$ctx/built-in-drivers/"
  else
    echo "  WARN: $PACKAGE_BUILD_DIR/built-in-drivers not found, images will not bundle drivers"
  fi
}
