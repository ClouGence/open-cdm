...
stage_build_context() {
  ...
  # built-in drivers baked into the image layer (generated by pkg-builtin-drivers:generateBuiltinDrivers)
  # bundle lives under package/pkg/builtin-drivers/bundle so `rm -rf package/build`
  # in package.sh --build does not wipe the hash-gated cache. Set
  # CG_BUILTIN_DRIVERS_BUNDLE_DIR to point at a shared cache across worktrees.
  local drivers_dir="${CG_BUILTIN_DRIVERS_BUNDLE_DIR:-$PACKAGE_DIR/pkg/builtin-drivers/bundle}"
  mkdir -p "$ctx/built-in-drivers"
  if [ -d "$drivers_dir" ]; then
    cp -R "$drivers_dir/." "$ctx/built-in-drivers/"
  else
    echo "  WARN: $drivers_dir not found, images will not bundle drivers"
  fi
}
