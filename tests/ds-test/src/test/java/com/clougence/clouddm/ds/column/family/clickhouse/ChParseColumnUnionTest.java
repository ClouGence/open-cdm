...
public class ChParseColumnUnionTest extends ChSelectColumnTestSupport {

    public ChParseColumnUnionTest(){
        spi = new ChSelectColumnAnalysisSpi(new TestMetaServiceImpl());
    }

    @Test
    public void test1() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select * from table1 union all select * from table2", createContextInfo());
    ...
    @Test
    public void test2() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select column1 from (select column1  from table1 union all select column2 from table2)c", createContextInfo());
    ...
    @Test
    public void test3() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select e from (select column1 as e  from table1 union all select column2 as x from table2)c", createContextInfo());
        assert selectItems.size() == 1;
    ...
    @Test
    public void test4() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select * from table1 union all select * from table2 union all select * from schema2.table1", createContextInfo());
        assert selectItems.size() == 6;
        for (int i = 1; i <= selectItems.size(); i++) {
            SelectItem selectItem = selectItems.get(i - 1);
            assert selectItem.getItemAlias().equals("column" + i);
    ...
    @Test
    public void test5() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select * from (select * from table1 union all select * from table2) c", createContextInfo());
        assert selectItems.size() == 6;
        for (int i = 1; i <= selectItems.size(); i++) {
            SelectItem selectItem = selectItems.get(i - 1);
