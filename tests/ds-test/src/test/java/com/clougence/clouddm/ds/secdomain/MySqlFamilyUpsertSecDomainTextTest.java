...
public final class MySqlFamilyUpsertSecDomainTextTest {
    ...
    public Stream<DynamicTest> upsertClassification() {
        return DATASOURCES.stream().flatMap(datasource -> {
            ...
            return SecDomainTextTest.loadCases(RESOURCE).stream().map(testCase -> DynamicTest.dynamicTest(
                    testCase.displayName(datasource),
                    () -> SecDomainTextTest.assertCase(RESOURCE, testCase, SqlTestSupport.dataSourceType(datasource), spi, context)));
        });
    }
}
