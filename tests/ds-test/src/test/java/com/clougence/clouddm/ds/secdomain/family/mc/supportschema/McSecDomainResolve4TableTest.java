...
import com.clougence.clouddm.ds.maxcompute.sql.security.domain.McColumnDomain;
import com.clougence.clouddm.ds.maxcompute.sql.security.domain.McSelectDomain;
import com.clougence.clouddm.ds.maxcompute.sql.security.domain.McTableDomain;
...
public class McSecDomainResolve4TableTest extends McSecDomainTestSupport {
    ...
    public void createTableIfNotExists_1() {
        String sql = "create table if not exists a.test.abc(id int) PARTITIONED BY (sale_date STRING, region STRING)";

        List<ResObject> resList = parserRes(sql);
        assert resList.size() == 2;
        {
            assert resList.get(0).getType() == TargetType.Table &&//
                   resList.get(0).toDsResPath().getResPath().equals("/a/test/abc/");
            assert resList.get(1).getType() == TargetType.Column &&//
                   resList.get(1).toDsResPath().getResPath().equals("/a/test/abc/id/");
        }
        ...
        assert domainList.size() == 2;
        {
            McTableDomain domain = (McTableDomain) domainList.get(0);
            assert domain.getSqlType() == SecQueryType.CREATE_TABLE && domain.getAuditKind() == SecQueryKind.CREATE;
            assert domain.getCatalog().equals("a") && domain.getSchema().equals("test") && domain.getTable().equals("abc") &&//
    ...
    public void createTableComplex_1() {
        ...
        {
            assert resList.get(0).getType() == TargetType.Table &&//
                   resList.get(0).toDsResPath().getResPath().equals("/test_db/test_schema/test_default/");
        }
        ...
        assert domainList.size() == 13;
        {
            McTableDomain domain1 = (McTableDomain) domainList.get(0);
            assert domain1.getSqlType() == SecQueryType.CREATE_TABLE && domain1.getAuditKind() == SecQueryKind.CREATE;
            assert domain1.getCatalog() == null && domain1.getSchema() == null && domain1.getTable().equals("test_default");
