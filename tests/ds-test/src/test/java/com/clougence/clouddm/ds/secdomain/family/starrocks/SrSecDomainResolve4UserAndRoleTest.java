...
import com.clougence.sql.doris.security.domain.DrGrantDomain;
...
import com.clougence.clouddm.ds.starrocks.sql.security.domain.SrGrantDomain;
import com.clougence.clouddm.ds.starrocks.sql.security.domain.SrRevokeDomain;
import com.clougence.clouddm.ds.starrocks.sql.security.domain.SrUserDomain;
...
public class SrSecDomainResolve4UserAndRoleTest extends SrSecDomainTestSupport {

    public SrSecDomainResolve4UserAndRoleTest(){
    ...
    //    @Test
    public void grant() {
        String sql = "GRANT SELECT_PRIV ON mydatabase.* TO 'test_user'@'localhost';";
    ...
    public void grantAll() {
        String sql = "GRANT ALL  ON mydatabase.* TO 'test_user'@'localhost';";
        ...
        assert domainList.size() == 1;
        {
            SrGrantDomain domain = (SrGrantDomain) domainList.get(0);
            assert domain.getSqlType() == SecQueryType.GRANT && domain.getAuditKind() == SecQueryKind.ALTER;
            assert domain.getHost().equals("localhost") && domain.getName().equals("test_user");
        }
    }
    ...
    @Test
    public void revoke() {
        String sql = "REVOKE ALL ON mydatabase.* FROM 'test_user'@'localhost';";
        ...
        assert domainList.size() == 1;
        {
            SrRevokeDomain domain = (SrRevokeDomain) domainList.get(0);
