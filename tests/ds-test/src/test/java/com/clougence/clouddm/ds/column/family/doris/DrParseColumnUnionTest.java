...
public class DrParseColumnUnionTest extends DrSelectColumnTestSupport {

    public DrParseColumnUnionTest(){
        spi = new DrSelectColumnAnalysisSpi(new TestMetaServiceImpl());
    }

    @Test
    public void test1() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select * from table1 union select * from catalog2.schema1.table2", createContextInfo());
    ...
    public void test2() {
        List<SelectItem> selectItems = spi
            .parseSelectColumn("select column1 from (select column1  from table1 union select column2 from catalog2.schema1.table2)c", createContextInfo());
    ...
    public void test3() {
        List<SelectItem> selectItems = spi
            .parseSelectColumn("select e from (select column1 as e  from table1 union select column2 as x from catalog2.schema1.table2)c", createContextInfo());
        assert selectItems.size() == 1;
    ...
    public void test4() {
        List<SelectItem> selectItems = spi
            .parseSelectColumn("select * from table1 union select * from catalog2.schema1.table2 union select * from schema2.table1", createContextInfo());
        assert selectItems.size() == 6;
        for (int i = 1; i <= selectItems.size(); i++) {
            SelectItem selectItem = selectItems.get(i - 1);
            assert selectItem.getItemAlias().equals("column" + i);
    ...
    @Test
    public void test5() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select * from (select * from table1 union select * from catalog2.schema1.table2) c", createContextInfo());
        assert selectItems.size() == 6;
        for (int i = 1; i <= selectItems.size(); i++) {
            SelectItem selectItem = selectItems.get(i - 1);
