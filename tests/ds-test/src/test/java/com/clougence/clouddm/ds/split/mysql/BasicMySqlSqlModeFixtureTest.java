 ...
 */
public abstract class BasicMySqlSqlModeFixtureTest extends BasicSplitTextTest {

    private static final String       FIXTURE_DELIMITER = "------------------------------------------------------------------------------------------";
    private static final ObjectMapper JSON              = new ObjectMapper();
    private final TextResourceShard   fixtureShard;

    protected BasicMySqlSqlModeFixtureTest(int shardCount, int shardId){
        this.fixtureShard = new TextResourceShard("split/mysql", shardCount, shardId);
    }
    ...
    public Stream<DynamicTest> modeSensitiveRejections() {
        return rejectedDynamicTests(fixtureShard.resourceFiles(path -> {
            return path.contains("/mode/") &&  //
                   path.endsWith(".txt") &&    //
                   path.contains("/reject/");
        }), "mysql");
    ...
    protected void splitRejectedCase(String resourcePath, String datasource, String rejectedSql) throws Exception {
        ...
        try (StringReader reader = new StringReader(rejectedSql)) {
            DslHelper.splitDsl(provider, reader);
        }
    }
    ...
    private static ParseEvidence evidence(MyDslProvider provider, String input) {
        ...
        try (StringReader reader = new StringReader(input)) {
            trees = DslHelper.splitDsl(provider, reader).stream().map(AstSplitScript::getAstTree).toList();
        }
...
final class MySqlSqlModeFixtureShard1Test extends BasicMySqlSqlModeFixtureTest { MySqlSqlModeFixtureShard1Test(){ super(3, 0); } }
final class MySqlSqlModeFixtureShard2Test extends BasicMySqlSqlModeFixtureTest { MySqlSqlModeFixtureShard2Test(){ super(3, 1); } }
final class MySqlSqlModeFixtureShard3Test extends BasicMySqlSqlModeFixtureTest { MySqlSqlModeFixtureShard3Test(){ super(3, 2); } }
