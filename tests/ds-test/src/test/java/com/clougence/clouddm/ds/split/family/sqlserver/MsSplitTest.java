package com.clougence.clouddm.ds.split.family.sqlserver;
...
public class MsSplitTest extends SplitTestSupport {
    ...
    @Test
    public void test4() {
        String sql = "alter table abc drop constraint abc_pk; select * from test; drop table rrw";
        List<SplitScript> splitScripts = this.splitAnalysisSpi.splitScript(sql, null, 0, 0);
        assert splitScripts.size() == 3;
        SplitScript splitScript = splitScripts.get(0);
        assert splitScript.getScript().equals("alter table abc drop constraint abc_pk;");
        assert splitScript.getType() == SecQueryType.ALTER_TABLE;

        splitScript = splitScripts.get(1);
        assert splitScript.getScript().equals("select * from test;");
        assert splitScript.getType() == SecQueryType.SELECT;

        splitScript = splitScripts.get(2);
        assert splitScript.getScript().equals("drop table rrw");
