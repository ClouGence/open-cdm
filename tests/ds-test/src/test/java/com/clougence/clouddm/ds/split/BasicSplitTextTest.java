...
public abstract class BasicSplitTextTest {
    ...
    protected final Stream<DynamicTest> rejectedDynamicTests(List<String> resourcePaths, String datasource) {
        return resourcePaths.stream().map(resourcePath -> {
            return DynamicTest.dynamicTest(resourcePath, () -> {
    ...
    private Stream<Fixture> fixtures() {
        return fixtureResources().stream().map(BasicSplitTextTest::toFixture);
    }
