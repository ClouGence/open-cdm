...
public class DrParseColumnSelectInColumnTest extends DrSelectColumnTestSupport {
    ...
    public DrParseColumnSelectInColumnTest(){
        spi = new DrSelectColumnAnalysisSpi(new TestMetaServiceImpl());
    }

    @Test
    public void test1() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select (select column1 from table1) as x from catalog2.schema1.table2", createContextInfo());
