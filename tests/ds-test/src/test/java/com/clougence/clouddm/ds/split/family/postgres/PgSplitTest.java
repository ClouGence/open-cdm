package com.clougence.clouddm.ds.split.family.postgres;
...
public class PgSplitTest extends SplitTestSupport {
    ...
    @Test
    public void test4() {
        String sql = "select * from table_1 a join table_2 b on a.id = b.id;with tab1Cnt as (select * from table1)"
                     + "update table2 set cc = 1 where id1 in (select id_1 from tab1Cnt) or id2 in (select id_2 from table3)";
        List<SplitScript> splitScripts = this.splitAnalysisSpi.splitScript(sql, null, 0, 0);
        assert splitScripts.size() == 2;
        SplitScript splitScript = splitScripts.get(0);
        assert splitScript.getScript().equals("select * from table_1 a join table_2 b on a.id = b.id;");
        assert splitScript.getType() == SecQueryType.SELECT;

        splitScript = splitScripts.get(1);
        assert splitScript.getScript()
            .equals("with tab1Cnt as (select * from table1)" + "update table2 set cc = 1 where id1 in (select id_1 from tab1Cnt) or id2 in (select id_2 from table3)");
