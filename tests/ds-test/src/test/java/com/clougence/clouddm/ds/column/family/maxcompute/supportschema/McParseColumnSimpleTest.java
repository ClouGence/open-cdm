...
public class McParseColumnSimpleTest extends McSelectColumnTestSupport {
    ...
    @Test
    public void test9() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select *,test(column1) as s from catalog2.schema2.table2", contextInfo());
        assert selectItems.size() == 7;
        for (int i = 1; i <= 6; i++) {
            SelectItem selectItem = selectItems.get(i - 1);
            assert selectItem.getTableAlias().equals("table2") && selectItem.getItemAlias().equals("column" + i);
            for (RealColumn column : selectItem.getColumns()) {
                assert column.getColumn().equals("column" + i) && column.getSchema().equals("schema2") && column.getTable().equals("table2");
