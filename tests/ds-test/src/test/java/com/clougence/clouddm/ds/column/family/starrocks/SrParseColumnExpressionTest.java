...
public class SrParseColumnExpressionTest extends SrSelectColumnTestSupport {

    public SrParseColumnExpressionTest(){
        spi = new SrSelectColumnAnalysisSpi(new TestMetaServiceImpl());
    }

    @Test
    public void test1() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select 1 + 1 as x", contextInfo());
    ...
    @Test
    public void test2() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select 1 + column1 as c from table1", contextInfo());
    ...
    @Test
    public void test3() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select 1 + column1 + (select column1 from catalog2.schema1.table2 limit 1) as x from table1", contextInfo());
        assert selectItems.size() == 1;
        SelectItem selectItem = selectItems.get(0);
        assert selectItem.getTableAlias() == null && selectItem.getItemAlias().equals("x");
    ...
    public void test4() {
        List<SelectItem> selectItems = spi
            .parseSelectColumn("select 1 + column1 + (select column1 from catalog2.schema1.table2 limit 1) + test(column2) as xx from table1", contextInfo());
        assert selectItems.size() == 1;
        SelectItem selectItem = selectItems.get(0);
    ...
    @Test
    public void test5() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select 32 + column1 + test(2) + (select column1 from table1) as test from schema2.table1 a", contextInfo());
        assert selectItems.size() == 1;
    ...
    @Test
    public void test6() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select 1 as x", contextInfo());
        assert selectItems.size() == 1;
        SelectItem selectItem = selectItems.get(0);
        assert selectItem.getTableAlias() == null && selectItem.getItemAlias().equals("x");
        assert selectItem.getColumns().isEmpty();
