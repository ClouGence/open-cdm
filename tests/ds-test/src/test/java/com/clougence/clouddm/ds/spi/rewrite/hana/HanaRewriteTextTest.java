/* Copyright 2026 杭州开云集致科技有限公司 */
package com.clougence.clouddm.ds.spi.rewrite.hana;

import com.clougence.clouddm.ds.hana.sql.editor.rewrite.HanaRewriteSpi;
import com.clougence.clouddm.ds.spi.rewrite.RewriteTextCase;
import com.clougence.clouddm.ds.spi.rewrite.RewriteTextTest;
import com.clougence.clouddm.sdk.sql.editor.rewrite.RewriteSpi;

public final class HanaRewriteTextTest extends RewriteTextTest {

    public HanaRewriteTextTest(){
        super("spi/rewrite/hana");
    }

    @Override
    protected RewriteSpi rewriteSpi(RewriteTextCase testCase) {
        return new HanaRewriteSpi();
    }
}
