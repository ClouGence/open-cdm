...
public class McParseColumnWithTest extends McSelectColumnTestSupport {
    ...
    @Test
    public void test1() {
        List<SelectItem> selectItems = spi.parseSelectColumn("with test1 as (select * from table2) select * from test1", contextInfo());
    ...
    public void test2() {
        List<SelectItem> selectItems = spi
            .parseSelectColumn("with test1 as (select * from catalog2.table2) select * from (with test2 as (select * from table2) select * from test2)", contextInfo());
        assert selectItems.size() == 6;
        for (int i = 1; i <= 6; i++) {
            SelectItem selectItem = selectItems.get(i - 1);
    ...
    public void test3() {
        List<SelectItem> selectItems = spi
            .parseSelectColumn("select * from (with test1 as (select * from table2) select * from (with test1 as (select * from table2) select * from test1)) t", contextInfo());
        assert selectItems.size() == 6;
        for (int i = 1; i <= 6; i++) {
            SelectItem selectItem = selectItems.get(i - 1);
