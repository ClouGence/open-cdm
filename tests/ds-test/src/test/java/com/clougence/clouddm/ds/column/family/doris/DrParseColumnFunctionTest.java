...
public class DrParseColumnFunctionTest extends DrSelectColumnTestSupport {

    public DrParseColumnFunctionTest(){
        spi = new DrSelectColumnAnalysisSpi(new TestMetaServiceImpl());
    }

    @Test
    public void test1() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select test_function(1) as x from table1", createContextInfo());
    ...
    @Test
    public void test2() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select test_function(column1) as c from table1", createContextInfo());
        assert selectItems.size() == 1;
        SelectItem selectItem = selectItems.get(0);
    ...
    @Test
    public void test3() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select test_function(32) as z", createContextInfo());
        assert selectItems.size() == 1;
        SelectItem selectItem = selectItems.get(0);
    ...
    @Test
    public void test6() {
        List<SelectItem> selectItems = spi.parseSelectColumn("select test_func((select column1 from catalog2.schema1.table2))  as x from table1", createContextInfo());
        assert selectItems.size() == 1;
        SelectItem selectItem = selectItems.get(0);
        //        assert selectColumn.getTableAlias() == null && selectColumn.getColumnAlias() == null;
        assert selectItem.getColumns().size() == 1;
    ...
    public void test7() {
        List<SelectItem> selectItems = spi
            .parseSelectColumn("select test_func((select test_func1((select column1 from schema2.table1),column1) as x from catalog2.schema1.table2),column1) as w from table1", createContextInfo());
        assert selectItems.size() == 1;
        SelectItem selectItem = selectItems.get(0);
        //        assert selectColumn.getTableAlias() == null && selectColumn.getColumnAlias() == null;
