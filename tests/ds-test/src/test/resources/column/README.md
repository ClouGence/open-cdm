...
`column` 用于验证 SQL 查询列血缘分析。测试入口会扫描本目录下所有 `*.txt`，按数据源目录执行对应 `SelectColumnAnalysisSpi`。
...
不要在 `column` 目录下新增私有 meta。新增 SQL 涉及的新表或列，应先补充 `_meta` 中对应 layer 的资源文件。

## 运行

```bash
./gradlew :s-test:test --tests 'com.clougence.clouddm.ds.column.ColumnTextTest' --no-daemon
```
