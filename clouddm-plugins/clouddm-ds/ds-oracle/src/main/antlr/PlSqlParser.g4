...
sql_script
    :  sql_plus_command_no_semicolon? (
        (SEMICOLON | sql_plus_command | unit_statement) (SEMICOLON '/'? (SEMICOLON | sql_plus_command | unit_statement))* SEMICOLON? '/'?
    ) EOF
    ;
...
create_procedure_body
    : CREATE (OR REPLACE)? (EDITIONABLE | NONEDITIONABLE)? PROCEDURE procedure_name ('(' parameter (',' parameter)* ')')? invoker_rights_clause? PARALLEL_ENABLE? (
        IS
        | AS
...
create_index
    : CREATE (unique_ | BITMAP)? INDEX index_name (IF NOT EXISTS)? ON (
        cluster_index_clause
        | tableview_name table_alias? table_index_columns
...
table_index_columns
    :  '(' index_expr (ASC | DESC)? (',' index_expr (ASC | DESC)?)* ')' index_properties?
    ;
...
identified_by
    : IDENTIFIED BY (id_expression | numeric)
    ;
...
create_view
    : CREATE (OR REPLACE)? (NO? FORCE)? editioning_clause? VIEW tableview_name (
        IF NOT EXISTS
...
out_of_line_constraint
    : (
        ((CONSTRAINT | CONSTRAINTS) constraint_name)? (
            UNIQUE constraint_column_list
            | PRIMARY KEY constraint_column_list
            | FOREIGN KEY constraint_column_list references_clause on_delete_clause?
            | CHECK '(' condition ')'
        )
    )
    constraint_state?
...
create_mv_refresh
    : (
        ...
        | REFRESH (
            (FAST | COMPLETE | FORCE)
            | ON (DEMAND | COMMIT)
            | (START WITH expression| NEXT expression) //date goes here TODO
...
drop_table
    : DROP TABLE tableview_name ((CASCADE CONSTRAINTS) | PURGE)?
    ;
...
comment_on_column
    : COMMENT ON COLUMN column_name IS comment_context
    ;
...
create_synonym
    ...
    | CREATE (OR REPLACE)? (EDITIONABLE | NONEDITIONABLE)? SYNONYM (synSchema=schema_name PERIOD)? synonym_name FOR (schema_name PERIOD)? schema_object_name (
...
comment_on_table
    : COMMENT ON TABLE tableview_name IS comment_context
    ;
...
modify_col_properties
    : column_name datatype? col_default? (ENCRYPT encryption_spec | DECRYPT)? inline_constraint* lob_storage_clause?
...
column_definition
    : column_name ((datatype | custom_type) (COLLATE column_collation_name)?)? SORT? (
        VISIBLE
        | INVISIBLE
    )? (col_default | identity_clause)? (ENCRYPT encryption_spec)? (
        inline_constraint+
        | inline_ref_constraint
    )?
    ;
...
constraint_clauses
    : ADD '(' (out_of_line_constraint (',' out_of_line_constraint)* | out_of_line_ref_constraint) ')'  #add_table_constarint
    | ADD (out_of_line_constraint | out_of_line_ref_constraint)         #add_table_constarint
    | MODIFY (
        CONSTRAINT constraint_name
        | PRIMARY KEY
        | UNIQUE '(' column_name (',' column_name)* ')'
    ) constraint_state CASCADE?                      #modify_table_constarint
    | RENAME CONSTRAINT old_constraint_name TO new_constraint_name    #rename_table_constarint
    | drop_constraint_clause+                   #drop_table_constarint
    ;
...
subquery_operation_part
    : (union_context) subquery_basic_elements
    ;
...
query_block
    : with_clause? SELECT (DISTINCT | UNIQUE | ALL)? selected_list into_clause? from_clause? where_clause? (
...
join_clause
    : query_partition_clause? join_type table_ref_aux query_partition_clause? (
        join_on_part
        | join_using_part
    )*
...
join_type
    : (CROSS | NATURAL)? (INNER | outer_join_type)? JOIN
    ;
...
insert_into_clause
    : INTO general_table_ref (LEFT_PAREN insert_column_list RIGHT_PAREN)?
    ;
...
numeric_function
    : SUM '(' (DISTINCT | ALL)? argument ')'
    | COUNT '(' (ASTERISK | ((DISTINCT | UNIQUE | ALL)? argument)?) ')' over_clause?
    | ROUND '(' argument (',' UNSIGNED_INTEGER)? ')'
    | AVG '(' (DISTINCT | ALL)? argument ')'
    | MAX '(' (DISTINCT | ALL)? argument ')'
    | LEAST '(' function_argument ')'
    | GREATEST '(' function_argument ')'
    ;
...
general_element_part
    ...
    | (INTRODUCER char_set_name)? routine_name ('@' link_name)? function_argument*
    ;
...
keyword_function:
    SYSDATE | SYSTIMESTAMP | LOCALTIMESTAMP |CURRENT_DATE
    ;
