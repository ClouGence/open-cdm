 ...
 */
@Slf4j
public class JtdsSqlServerDsFactory implements DsFactory<Connection> {
    ...
    public DsObject<Connection> create(Properties dsConfig) throws SQLException {
        ...
        if (StringUtils.isNotBlank(tcpKeepAlive)) {
            props.put("sokeepalive", tcpKeepAlive);
        }
    ...
    protected Connection createConnection(String jdbcUrl, Properties dsConfig) throws SQLException {
        return new net.sourceforge.jtds.jdbc.Driver().connect(jdbcUrl, dsConfig);
    }
}
