...
import com.clougence.clouddm.ds.tidb.parser.antlr.TiDBLexer;
...
public class TiDBDslProvider implements DslProvider {
    ...
    public static final DslProvider    INSTANCE    = new TiDBDslProvider();
    private final AntlrStatementParser TREE_PARSER = new TiDBAntlrStatementParser();

    @Override
    public String[] getDslName() { return new String[] { "TiDB", "TiDB SQL Language" }; }

    @Override
    public Lexer createLexer(CharStream charStream) {
        return new TiDBLexer(charStream);
    }

    @Override
    public Parser createParser(Lexer lexer) {
        return new TiDBParser(new CommonTokenStream(lexer));
    }
