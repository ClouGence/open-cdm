 ...
 */
@Slf4j
public class PorPgMetaProviderUtils extends PgMetaProviderUtils {
    ...
    protected FieldType safeToPostgresTypes(long serverVersionNumber, ResultSet rs) throws SQLException {
        ...
        PolarDBPgTypes pgTypeEnum = PolarDBPgTypes.valueOfTypeOid(typeOid.intValue());
        if (pgTypeEnum == null) {
            pgTypeEnum = PolarDBPgTypes.valueOfTypeOid(typeOid);
            if (pgTypeEnum == null) {
                pgTypeEnum = PolarDBPgTypes.valueOfCode(typeName);
            }
        }
