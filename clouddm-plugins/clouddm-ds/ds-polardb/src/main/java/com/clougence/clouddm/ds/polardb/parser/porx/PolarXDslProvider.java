...
public class PolarXDslProvider implements DslProvider {

    public static final DslProvider    INSTANCE    = new PolarXDslProvider();
    private final AntlrStatementParser TREE_PARSER = new PolarXStatementParser();

    @Override
    public String[] getDslName() { return new String[] { "PolarDB-X", "PolarDB-X SQL Language" }; }

    @Override
    public Lexer createLexer(CharStream charStream) {
        return new PolardbXLexer(charStream);
    }

    @Override
    public Parser createParser(Lexer lexer) {
        return new PolardbXParser(new CommonTokenStream(lexer));
    }
