...
*/
@Slf4j
public class PorXEditorProvider extends MyEditorProvider {

    public static final SqlBuilder INSTANCE = new PorXEditorProvider();

    @Override
    public DsType getDataSourceType() { return DsType.PolarDbX; }
    ...
    public List<String> tableCreate(TriggerContext buildContext, String catalog, String schema, String table, ETable eTable) {
        return new PorXCreateUtils().buildCreate(buildContext, catalog, schema, table, eTable);
    }
    ...
    public List<String> addColumn(TriggerContext buildContext, String catalog, String schema, String table, EColumn columnInfo, ETable eTable) {
        ...
        sqlBuild.append(" " + PorXTypeUtils.buildColumnType(columnInfo, buildContext).toUpperCase());
        ...
        if (columnInfo.getDefaultValue() != null) {
            String defaultValue = PorXTypeUtils.buildDefault(sqlTypes, mainVersion, columnInfo, buildContext).toUpperCase();
    ...
    public List<String> columnChange(TriggerContext buildContext, String catalog, String schema, String table, EColumn columnInfo, EColumn newInfo, List<String> diffChange,
        ...
        boolean useDelimited = buildContext.isUseDelimited();
        StringBuilder sqlBuild = buildAlterTable(buildContext, catalog, schema, table);
        PolarDbXTypes sqlTypes = PolarDbXTypes.valueOfCode(newInfo.getDbType());
        MainVersion mainVersion = buildContext.getTarDsInfo().getMainVersion();
        List<String> result = new ArrayList<>();

        sqlBuild.append(" MODIFY COLUMN " + getDialect().fmtName(useDelimited, columnInfo.getName()));
        sqlBuild.append(" " + PorXTypeUtils.buildColumnType(newInfo, buildContext).toUpperCase());
        ...
        String defaultValue = PorXTypeUtils.buildDefault(sqlTypes, mainVersion, newInfo, buildContext);
