...
import static com.clougence.adapter.doris.DorisAttributeNames.*;
 ...
 **/
@Slf4j
public class DrMetaProviderUtils extends MyMetaProviderUtils {
    ...
    public List<Value> convertExtTableName(ResultSet rs) throws SQLException {
        List<Value> result = new ArrayList<>();
        while (rs.next()) {
            RdbValue schema = new RdbValue();
            schema.setValue(rs.getString(1));
    ...
    public List<Value> convertCatalogName(ResultSet rs) throws SQLException {
        ...
        while (rs.next()) {
            ...
            if (!type.equals("internal")) {
                schema.setUmiType(UmiTypes.ExternalCatalog);
            } else {
    ...
    public List<Value> convertSchema(String catalog, ResultSet rs) throws SQLException {
        List<Value> result = new ArrayList<>();
        while (rs.next()) {
            RdbValue schema = new RdbValue();
            schema.setValue(rs.getString(1));
