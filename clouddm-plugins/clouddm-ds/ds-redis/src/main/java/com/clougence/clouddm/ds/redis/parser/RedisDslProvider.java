...
import com.clougence.clouddm.ds.redis.parser.antlr.RedisLexer;
...
public class RedisDslProvider implements DslProvider {

    public static final DslProvider    INSTANCE    = new RedisDslProvider();
    private final AntlrStatementParser TREE_PARSER = new RedisAntlrStatementParser();

    @Override
    public String[] getDslName() { return new String[] { "Redis", "Redis Command Language" }; }

    @Override
    public Lexer createLexer(CharStream charStream) {
        return new RedisLexer(charStream);
    }

    @Override
    public Parser createParser(Lexer lexer) {
        return new RedisParser(new CommonTokenStream(lexer));
    }
    ...
    public StatementSet doParser(Lexer lexer, Parser parser) {
        DefaultRedisParserVisitor visitor = new DefaultRedisParserVisitor();
        return (RedisCmdSet) visitor.visit(((RedisParser) parser).rootInstSet());
    }
