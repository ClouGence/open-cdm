...
import com.clougence.clouddm.ds.oceanbase.parser.ob4ora.antlr.ObForOracleLexer;
...
public class ObOraDslProvider implements DslProvider {

    public static final DslProvider    INSTANCE    = new ObOraDslProvider();
    private final AntlrStatementParser TREE_PARSER = new ObOraStatementParser();
    ...
    public Lexer createLexer(CharStream charStream) {
        return new ObForOracleLexer(charStream);
    }

    @Override
    public Parser createParser(Lexer lexer) {
        return new ObForOracleParser(new CommonTokenStream(lexer));
    }
