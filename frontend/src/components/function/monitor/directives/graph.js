...
import { updateLegendValues } from '../core/time_series2';
import GraphTooltip from '../panel/graph/tooltip';
import { ThresholdManager } from '../panel/graph/threshold_manager';
import { EventManager } from '../panel/annotations/event_manager';
import { convertToHistogramData } from '../panel/graph/histogram';
import { alignYLevel } from '../panel/graph/align_yaxes';
...
export default function link(scope, elem) {
  ...
  const tooltip = new GraphTooltip(elem, dashboard, scope, (() => sortedSeries));
  ...
  function getMinTimeStepOfSeries(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      } else if (typeof data[i].bars === 'undefined' || typeof data[i].bars.show === 'undefined' || !data[i].bars.show) {
        continue;
      }
  ...
  function render_panel() {
    ...
    // Populate element
    const options = buildFlotOptions(panel);
  ...
  function prepareXAxis(options, panel) {
    switch (panel.xaxis.mode) {
      ...
      case 'histogram': {
        ...
        if (data.length) {
          const histMin = _.min(_.map(data, (s) => s.stats.min));
          const histMax = _.max(_.map(data, (s) => s.stats.max));
  ...
  function addXSeriesAxis(options) {
    const ticks = _.map(data, (series, index) => [index + 1, series.alias]);
  ...
  function addXTableAxis(options) {
    let ticks = _.map(data, (series, seriesIndex) => _.map(series.datapoints, (point, pointIndex) => {
  ...
  elem.bind('plotselected', (event, ranges) => {
