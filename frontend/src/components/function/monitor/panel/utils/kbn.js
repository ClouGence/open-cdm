...
kbn.secondsToHhmmss = function (seconds) {
  ...
  numhours > 9 ? strings.push(String(numhours)) : strings.push(`0${numhours}`);
  numminutes > 9 ? strings.push(String(numminutes)) : strings.push(`0${numminutes}`);
  numseconds > 9 ? strings.push(String(numseconds)) : strings.push(`0${numseconds}`);
  return strings.join(':');
};
...
kbn.toDuration = function (size, decimals, timeScale) {
  ...
  size
        *= kbn.intervals_in_seconds[
      units.find((e) => e.long === timeScale).short
    ] * 1000;
