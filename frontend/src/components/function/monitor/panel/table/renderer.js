...
export class TableRenderer {
  ...
  createColumnFormatter(column) {
    ...
    return (value) => this.defaultCellFormatter(value, column.style);
  }
  ...
  renderCell(columnIndex, rowIndex, value, addWidthHack = false) {
    ...
    if (column.style && column.style.link) {
      ...
      scopedVars.__cell = { value };

      const cellLink = this.templateSrv.replace(column.style.linkUrl, scopedVars);
