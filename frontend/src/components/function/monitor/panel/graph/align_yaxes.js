...
export function alignYLevel(yAxes, level) {
  ...
  if (zero && oneSide) {
  ...
  } else if (checkOppositeSides(yLeft, yRight)) {
    if (yLeft.min >= 0) {
      yLeft.min = -yLeft.max;
    ...
    } else if (checkTwoCross(yLeft, yRight)) {
...
function getRate(yLeft, yRight) {
  ...
  } else if (yLeft.min > 0 || yRight.min > 0) {
