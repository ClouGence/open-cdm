...
export class DataProcessor {
  ...
  getSeriesList(options) {
    ...
    switch (this.panel.xaxis.mode) {
      case 'series':
      case 'time': {
        return options.dataList.map((item, index) => this.timeSeriesHandler(item, index, options));
      }
      case 'histogram': {
        ...
        return histogramDataList.map((item, index) => this.timeSeriesHandler(item, index, options));
      }
      case 'field': {
  ...
  getDataFieldNames(dataList, onlyNumbers) {
    ...
    function getPropertiesRecursive(obj) {
      _.forEach(obj, (value, key) => {
        ...
        } else if (!onlyNumbers || _.isNumber(value)) {
